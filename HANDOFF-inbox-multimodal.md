...
| 2 — Composer WhatsApp (anexo/áudio/emoji) | 🛠 em execução (T1-T2 ✅, T3-T5 em curso) | T1 upload storage-first (guard DoS); T2 backend envia mídia via signed URL 600s + wahaSendPlanFor — review pegou e matou IDOR cross-tenant de media_storage_path (anti-pattern #10 do CLAUDE.md) antes de qualquer deploy. |

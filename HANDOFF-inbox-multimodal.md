...
| 3 — Agente multimodal (model-agnóstico) | 🛠 em execução (T1-T6 ✅) | Migration 0058 (derivado+flags); capability registry; openai+google registrados; TranscriptionProvider; deriveMediaText; worker media_derive_v1 (BYOK, vision+whisper+pdf). Falta wiring no contexto+turno (T7-T8) e prova ≥2 modelos (T9). Vídeo → Onda 3.1 (ffmpeg). |

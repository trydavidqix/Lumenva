...
| 2 — Composer WhatsApp (anexo/áudio/emoji) | ✅ código completo + prova CRM→WAHA (aguarda confirmação do Rafael no celular) | T1 upload storage-first (guard DoS); T2 backend envia mídia via signed URL 600s + wahaSendPlanFor — review pegou e matou IDOR cross-tenant de media_storage_path (anti-pattern #10 do CLAUDE.md) antes de qualquer deploy. |

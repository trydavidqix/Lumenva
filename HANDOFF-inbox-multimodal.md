...
| 3 — Agente multimodal (model-agnóstico) | ✅ CÓDIGO COMPLETO (8/8 + review final Ready) — prova T9 aguarda chaves BYOK reais | 2 camadas: (universal) worker `media_derive_v1` preenche `media_derived_text` (transcrição Whisper/visão/pdf), lido por `get-lead-context` → qualquer modelo lê; (aprimoramento) parte nativa AI SDK capability-gated em `inbound-turn` só p/ a mídia do turno atual, degrada sem quebrar. Providers anthropic+openai+google registrados (seam passa ModelMessage[] cru). Migration 0058. 451 testes, tsc/lint 0. **Prova bloqueada**: credencial Anthropic atual é placeholder (AES decrypt falha `Unsupported state`); Rafael vai cadastrar Anthropic+OpenAI+Google reais em /app/ai/credentials. Vídeo → Onda 3.1 (ffmpeg). |

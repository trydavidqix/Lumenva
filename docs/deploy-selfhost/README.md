...
| Usuário confirma o e-mail e loga, mas cai em "Você não tem nenhuma organização ativa" | Passo 3.5, item 3 não foi feito — o projeto ainda usa o template default do Supabase (`{{ .ConfirmationURL }}`, link `?code=`), incompatível com `app/auth/confirm/route.ts` (espera `?token_hash=&type=`). O e-mail confirma normalmente (o Supabase processa o `code` no próprio domínio antes de redirecionar), mas `ensureTenantForUser()` nunca roda porque a rota nunca reconhece o link — sem erro visível nos logs do app | refaça o passo 3.5 item 3 nesse projeto; para o usuário já afetado, a organização precisa ser criada manualmente (`organizations` + `user_organizations` com `role='admin'`, mesma lógica de `lib/auth/provision.ts`) |

...
O vocabulário usa exatamente dois segmentos (`domínio.evento`), separados por um ponto; o
segmento do evento usa sublinhados para ações compostas. O contrato de aplicação usa
`camelCase`; `toContentEventLogRecord` persiste `snake_case`, mantém `organization_id` e
`entity_id` no payload auditável e grava `metadata.request_id`.

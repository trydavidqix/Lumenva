...
| **Operação visível** (`HANDOFF-operacao-visivel.md`) | F1, F2(i), F2(ii), F3 ✅ localhost com evidência Playwright. **Prova na VPS refeita 2026-08-25** (o HANDOFF tinha provado contra servidor errado, `129.121.45.100:18080`, não é produção): F1 ✅ passou em `crm.lumenva.pt` real (rota, RBAC, build confirmados). F3 investigado a fundo — aba "Propostas" some no agente de teste porque propostas são org-scoped e o agente testado é de outro tenant; **confirmado comportamento esperado, não bug** (13 propostas reais existem, só que no tenant Lumenva) | F2(ii) e F2(i) seguem sem prova de UI na VPS real — precisa de tenant com WhatsApp conectado e dado seedado, não tentado por instrução de não mutar produção |

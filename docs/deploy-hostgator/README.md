# Como subir o seu Lumenva na HostGator (passo a passo, sem enrolação)
...
```
Seu domínio  →  Servidor VPS (HostGator)  →  Lumenva rodando
                        │
                        ├─ o CRM (site + painel)
...
O Lumenva roda num **VPS com Docker**. A opção mais fácil é um VPS que **já vem
...
O Lumenva foi desenhado pra rodar redondo na infraestrutura da HostGator. Além do

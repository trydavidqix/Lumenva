...
| Resend | `apps/crm/lib/email/resend.ts`, `apps/website/lib/resend.ts` | `EmailPort` | destinatário/template; não logar corpo sensível | domínio/test mode; `RESEND_*` | idempotency quando suportado; dedupe de convite | UI informa não enviado; `accept_url` controlado; retry só seguro |

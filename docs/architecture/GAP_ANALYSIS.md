...
| Organization tenancy / RLS | EXISTS | `infra/supabase/migrations/`, `apps/crm/lib/auth/`, `apps/crm/lib/supabase/`, `organization_id` filters and RLS tests | Full cross-surface isolation still needs a single Phase-0 evidence index. |
...
| Memory / Customer Memory | PARTIAL | `apps/crm/lib/agent-engine/memory/`, `customer-memory/`, `infra/docker/mem0/`, Graphiti docs | Memory integrations and flags exist; canonical durable memory package and governance flow are incomplete. |
...
| Asset Intelligence | PARTIAL | media/creative components and `apps/video-composer/` | Media exists; OCR/vectorization/layers/reverse-design contract is not present as a governed asset engine. |
...
| Marketing + Video | PARTIAL | `apps/video-composer/`, content-os code, marketing docs | Content/video paths exist; blueprint end-to-end CMO/research/asset/QA pipeline is not one contract. |
...
| Infrastructure plane | PARTIAL | `infra/docker/`, `infra/supabase/`, `ops/`, `docs/runbooks/`, worker deployment paths | Operational infrastructure exists; canonical Business OS infra package and evidence graph are not unified. |

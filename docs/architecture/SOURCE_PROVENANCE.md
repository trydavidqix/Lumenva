...
| Tenant identity is `organization_id`; RLS is mandatory | security invariant | BLUEPRINT + REPO | `~/master-blueprint-IMPLEMENTAVEL.md:2.16`; `docs/specs/01-spec-platform-base.md`; `infra/supabase/migrations/` | CONFIRMED doctrine; implementation is distributed across migrations/policies |
...
| Current repo has mature CRM, audit, event-log, agent-engine, voice, LGPD and provider integrations | current capability | REPO | `apps/crm/lib/`, `apps/crm/app/api/`, `infra/supabase/`, `apps/crm/workers/`, `apps/voice-worker/`, `docs/current-state.md` | CONFIRMED by path inventory; completeness varies per module |

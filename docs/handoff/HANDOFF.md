...
- 2026-07-23: **Task 8.3 (jornada) ✅ — `tests/e2e/followup-journey.spec.ts`, a prova ponta-a-ponta que amarra
  ...
  **PROVA — 2 rodadas seguidas, síncronas, 1/1 verde cada** (porta 3033, `E2E_PORT=3033`): rodada 1 = 1m18s,
  ...
  `e2e-artifacts/followup-8.3-0{1..4}-*` (fluxo publicado, seção Follow-up do agente, fila com o
...
- 2026-07-22: **Task 7.2 ✅ — seletor de fluxo no editor do agente + gate de gatilho automático. Onda 7 fechada.**
  ...
  **ADMIN** (não manager — achado ao ler `page.tsx`: manager VÊ o form mas `readOnly=true`, só admin salva;
  ...
  screenshot `e2e-artifacts/followup-7.2-01-flow-selected`, clica "Salvar rascunho", screenshot
  `e2e-artifacts/followup-7.2-02-saved`, e prova via **API** (`GET .../versions/:vid`) que
...
- 2026-07-22: **Task 6.2 ✅ (commits e06bf1e/bb928fd/fbc2415/6fdce2d/3d5d9f4) — builder visual
  ...
  **PROVA — sequência mandatória completa** (Playwright headed, dev server real porta 3022 +
  ...
  (1 versão, é o 1º publish). Screenshots `test-results/followup-6.2-0{1..8}-*`.
...
- 2026-07-22: **Task 6.1 ✅ (commit fcd0068) — página + lista de fluxos, 1ª tela user-visible da feature.**
  ...
  **PROVA (Playwright headed, `tests/e2e/followup-builder.spec.ts`, 2/2 verde):** dev server real
  ...
  esconder o botão no client). 4 screenshots em `test-results/followup-6.1-0{1..4}-*` (visualmente

...
- Graph View visual, drill-down e endpoint dashboard estão entregues; falta ligar configuração de produção. `KnowledgeContextRetriever` lê apenas notas Obsidian `PUBLISHED` limpas, combina busca Graphiti e alimenta `CoreRuntime.requestProgressiveContext` com gates L0→L1→L2; 33/33 testes Core e typecheck PASS.

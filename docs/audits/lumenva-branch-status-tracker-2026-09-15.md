...
1. **Alto — hardening de entitlements amplia indevidamente a autoridade de membros do tenant.** A migration 0161 define `organization_plan_platform_write` como `for all` somente para `fn_is_platform_admin()` (linhas 51–52) e `entitlement_events` somente com leitura/inserção para membros (linhas 53–56). A migration 0170 adiciona `organization_plan_tenant_all` e `entitlement_events_tenant_all`, ambos `for all to authenticated`, com `using/with check` apenas no pertencimento à organização (linhas 10–20). Como as policies permissivas anteriores não são removidas, policies são combinadas por OR: qualquer usuário autenticado da organização pode inserir/alterar/remover sua atribuição de plano e editar/apagar eventos de entitlement. Isso permite autoelevação de módulos e adulteração do ledger de auditoria. Evidência: `nl -ba supabase/migrations/20260911100000_0161_entitlements_catalog.sql | sed -n '49,56p'` e `nl -ba supabase/migrations/20260915090000_0170_tenant_rls_hardening.sql | sed -n '10,20p'`. **Não corrigido nesta auditoria**, para preservar o escopo solicitado; requer migration corretiva e teste cross-tenant/role.

2. **Alto — `approval.ts` usa `compareAndSet` que não existe em `ApprovalStore`.** A interface declara somente `save` e `load` (linhas 32–35), mas três caminhos acessam `store.compareAndSet` (linhas 162–164, 187–189 e 251–257). A única ocorrência de implementação está num double/test fake (`wave1-policy-edges.test.ts`); não existe declaração no contrato nem store de produção encontrada por `rg`. Isso é erro estático `TS2339` potencialmente bloqueante do typecheck e também deixa o caminho sem CAS cair no `save` não atômico. Evidência: `rg -n 'compareAndSet' apps/crm --glob '*.ts'` e `nl -ba apps/crm/lib/agent-engine/policies/approval.ts | sed -n '32,35p;158,164p;187,192p;250,260p'`. **Não corrigido**, pois a tarefa foi auditoria, não implementação.

3. **Alto — decisão de approval continua load-then-save e pode perder a primeira decisão concorrente.** `decideApprovalRequest` lê o status, aceita qualquer `pending` e grava a decisão sem `compareAndSet` (linhas 125–140). O comentário afirma “First terminal decision wins”, mas duas instâncias podem ler `pending`, uma aprovar e outra negar, e a última gravação vence. O lock em memória adicionado a `enforceApprovalDecision` não protege `decideApprovalRequest` nem processos distintos. Evidência: `nl -ba apps/crm/lib/agent-engine/policies/approval.ts | sed -n '125,140p'`, ausência de `compareAndSet` nesse caminho e o lock local em linhas 197–212. **Não corrigido**; exige contrato/store durável e teste concorrente real.

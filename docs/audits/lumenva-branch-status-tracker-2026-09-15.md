...
3. **Alto — decisão de approval continua load-then-save e pode perder a primeira decisão concorrente.** `decideApprovalRequest` lê o status, aceita qualquer `pending` e grava a decisão sem `compareAndSet`. O lock em memória de `enforceApprovalDecision` não protege `decideApprovalRequest` nem processos distintos. **Correção em commit separado subsequente desta remediação**; o teste concorrente existente em `wave1-policy-edges.test.ts` será a prova de uma única decisão vencedora. A validação Vitest depende do gate Cloud porque o runner não está instalado localmente.

...
## Estado de execução em 2026-08-16
...
| 5 | `GO SEM ATIVAÇÃO EXTERNA`, FECHADA | [`phase-5-guardrails-gate.md`](../../evidence/ai-platform/phase-5-guardrails-gate.md), [`guardrails-gap-analysis.md`](../../evidence/ai-platform/guardrails-gap-analysis.md) | Tarefa 1 (gap analysis obrigatória pelo próprio plano) concluiu `NO EXTERNAL VALIDATOR NEEDED` — toda categoria de risco que um validador externo cobriria já tem detector nativo determinístico ou classificador LLM barato com teste passando (`ai:eval:local` 36/0dup/0P0, `test:unit` 332/3457 antes das tasks seguintes). Corte de escopo aprovado pelo humano: só Tarefas 2 (porta) e 3 (sanitizer de minimização) construídas como scaffolding reutilizável, nunca conectado a nenhum consumidor; Tarefas 4–7 (cliente REST, serviço Python self-hosted, integração no before-send, avaliação) puladas com evidência, permanecem especificadas no plano pra uso futuro. Revisão final whole-branch pesada dispensada pelo humano dado o baixo risco (scaffolding não-usado, zero mudança de runtime); cada task de código já teve review individual Approved, e varredura própria do controlador (`typecheck`/`lint`/`lint:channels`/`git diff --check`) achou e corrigiu o único defeito real (`any` implícito em teste). Nenhum flag, serviço ou secret novo — pode ficar assim indefinidamente sem custo operacional. Publicada em `origin/main` |
| 6 | `GO`, FECHADA | [`phase-6-n8n-gate.md`](../../evidence/ai-platform/phase-6-n8n-gate.md) | Tarefas 1–8 concluídas via subagent-driven-development (SDD). Implementação completa: n8n outbound envelope + HMAC signing, n8n→CRM scoped MCP actions, standalone docker-compose, 8 failure scenarios em golden cases. Sem revisão final whole-branch (cada task individualmente Approved, teste suite completo 3627/3627 passando, phase anterior gate completo). 139 testes novos (boundary invariants, envelope, scope isolation, workflow reference, failure resilience). Feature OFF por default, zero secrets novos, zero mudanças em código existente — opcional e não-invasivo. Pronta para merge em `origin/main`. Publicada em `origin/main` |
| 7 | não iniciada | — | gate da fase 6 e aprovação humana aplicável |

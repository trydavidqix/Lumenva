...
- M0.8 follow-up, dashboard wiring commit `6ca61708` (included in PR head `33cca25c`): MCG dashboard registry APIs refresh and serve six canonical registries (`agents`, `tools`, `plugins`, `mcps`, `runtimes`, `models`); `/api/views` exposes registry-backed Agents/Tools/Plugins/MCPs. Entries without evidence remain `UNAVAILABLE`, with null `last_seen` and unavailable measurement. Follow-up telemetry changes add outcome-aware ingestion, schema support, and evidence-based success/failure rates plus mean latency aggregation in registries. RED→GREEN tests cover outcome mapping and aggregation. Local checks: MCG 36/36, dashboard 14/14, contracts 1/1, syntax 30 modules, sensitive scan 80 files, `git diff --check` PASS. Actual provider config discovery and MCP/tool capability probes remain open; M0.8 and M0.13 are not closed.
- Extraction (2026-09-23): the complete MCG package was moved to the public canonical repository https://github.com/trydavidqix/maestri-context-gateway (initial commit `7dc2f71`). Standalone CI PASS. This Lumenva branch removes the duplicate package and its package-specific workflow; the Lumenva plan retains only roadmap/status and links to the new source. Provider discovery and MCP/tool capability probes remain open; M0.8/M0.13 remain incomplete.

...
- F17/F18 bridge increment: `operating-core` now owns the canonical snake_case `ContextPacket`; Core exports a deterministic `toDelegationContext` adapter for Obsidian/Graphiti-derived packets, and `MaestriDelegator` accepts that typed packet. Core context test: 8/8; delegator test: 2/2; typecheck passed.
- Provider wiring increment: `TaskContract.context_packet` is now the single handoff field; `MaestriDelegator` injects the resolved packet before `execute()`, and real Codex/Claude runners receive it through the serialized canonical contract. Operating-core validation: 26 passed, 2 Docker-dependent skips, typecheck passed.
- Core execution increment: `CoreRuntime.delegateTask()` now validates task identity, calls Maestri, persists the provider `ExecutionResult`, and exposes it through the existing execution feed/dashboard. Core validation: 43/43 tests and typecheck passed.
...
| MCG standalone público | `main` público em `trydavidqix/maestri-context-gateway`; PR #3 de extração das melhorias VPS, arquivo dos planos históricos e exports tipados integrado no merge `b3b5c8808b6f476654cce266f9d2cafd93a99193`. CI MCG verde; 46/46 testes, 33 módulos no syntax check e scan sensível aprovado. O Core VPS consome os subpaths tipados pela dependência Git pinada nesse SHA. | (1) testar discovery/probe em mais perfis Windows e clientes oficiais atuais; (2) enriquecer telemetria de erros MCP e capabilities de protocolo quando observáveis; (3) manter uso/taxas/latência indisponíveis até existir telemetria observada. O `docs/IMPLEMENTATION_PLAN.md` e `docs/archive/` do repo extraído são baselines históricos, não trackers atuais; usar `docs/STATUS.md`. |
...
| Checkout local do MCG | Repositório standalone em `C:\Users\David\Desktop\Projetos\maestri-context-gateway`; `main` agora contém runtime-root (PR #2) e a extração/integração tipada das melhorias VPS (PR #3). `.mcg-state/` local ignorado foi preservado. O diretório antigo `%USERPROFILE%\.lumenva\maestri-context-gateway` não é checkout canônico. | Manter o SHA da dependência do Core sincronizado com a versão aprovada no repo MCG; não mover/apagar estado local `.mcg-state/`. |

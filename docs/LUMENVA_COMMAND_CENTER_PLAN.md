...
| Checkout local do MCG | Repositório standalone em `C:\Users\David\Desktop\Projetos\maestri-context-gateway`, branch `codex/project-local-runtime-root`, commit `99ffa31` sincronizado com `origin`; `.mcg-state/` local ignorado foi preservado. A alteração passou 40/40 testes, syntax/import 31 módulos e sensitive/path scan. O diretório antigo `%USERPROFILE%\.lumenva\maestri-context-gateway` está vazio e não é checkout. | A mudança de runtime está publicada numa branch separada, ainda não integrada à `main` do repo MCG. Revisar e integrar somente em etapa autorizada; não mover/apagar o estado `.mcg-state/`. |

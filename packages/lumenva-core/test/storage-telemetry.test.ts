...
test('telemetry carries provenance and emits event',async()=>{const bus=new EventBus(); const seen:string[]=[]; bus.subscribe(e=>seen.push(e.type)); const t=new TelemetryStore(bus); await t.observe({name:'agent.duration',value:12,unit:'ms',source:'agent-runtime',provenance:'runtime'}); assert.equal(t.list()[0]?.provenance,'runtime'); assert.deepEqual(seen,['telemetry.observed']);});

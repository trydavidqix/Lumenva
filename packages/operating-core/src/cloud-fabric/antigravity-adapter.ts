import type { CapabilitySnapshot, ExecutionPort, ExecutionResult, QuotaSnapshot, TaskContract } from './execution-port';

export interface AntigravityExecutionClient {
  execute(contract: TaskContract): Promise<ExecutionResult>;
  quota?(): Promise<QuotaSnapshot>;
  health?(): Promise<'healthy' | 'degraded' | 'unavailable'>;
}

export class AntigravityAdapter implements ExecutionPort {
  public readonly name = 'antigravity';
  constructor(private readonly client?: AntigravityExecutionClient) {}

  async execute(contract: TaskContract): Promise<ExecutionResult> {
    if (!this.client) {
      return {
        task_id: contract.task_id,
        provider: this.name,
        status: 'blocked',
        summary: 'Antigravity runtime is not connected.',
        files_changed: [],
        tests: [],
        evidence: 'antigravity_runtime_unconfigured',
        risks: ['external_runtime_required'],
      };
    }
    return this.client.execute(contract);
  }

  async checkQuota(): Promise<QuotaSnapshot> {
    if (!this.client?.quota) {
      return {
        provider: this.name,
        tokens_used: 0,
        cost_usd: 0,
        health: 'degraded',
        measurement_type: 'unavailable',
        observed_at: new Date().toISOString(),
      };
    }
    return this.client.quota();
  }

  async getCapabilities(): Promise<CapabilitySnapshot> {
    return {
      provider: this.name,
      capabilities: ["planning","research","browser","vision","review","coding"],
      supports_tools: true,
      supports_vision: true,
      supports_browser: true,
      health: await this.health(),
      observed_at: new Date().toISOString(),
    };
  }

  async health(): Promise<'healthy' | 'degraded' | 'unavailable'> {
    if (!this.client) return 'degraded';
    return this.client.health ? this.client.health() : 'healthy';
  }
}

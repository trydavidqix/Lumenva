...
export class CodexAdapter implements ExecutionPort {
  ...
  constructor(private readonly client?: CodexExecutionClient) {}
  ...
  async execute(contract: TaskContract): Promise<ExecutionResult> {
    if (!this.client) {
      return {
        ...
        provider: this.name,
        status: 'blocked',
        summary: 'Codex runtime is not connected.',
        files_changed: [],
        tests: [],
        evidence: 'codex_runtime_unconfigured',
  ...
  async getCapabilities(): Promise<CapabilitySnapshot> {
    return {
      provider: this.name,
      capabilities: ["coding","review","architecture","debugging","tests"],
      supports_tools: true,

...
-- ---- Wave 8 Asset Intelligence: durable layer reuse approvals (migration 0161) ----
...
create index if not exists layer_reuse_approvals_org_status_idx
  on public.layer_reuse_approvals (organization_id, status, expires_at);
...
create policy layer_reuse_approvals_tenant_all on public.layer_reuse_approvals for all to authenticated
  using (organization_id in (select public.fn_user_org_ids()))
  with check (organization_id in (select public.fn_user_org_ids()));

...
CREATE POLICY "tenant_isolation_ai_budgets_all" ON "public"."ai_budgets" USING (("public"."fn_role_at_least"("organization_id", 'admin'::"text") OR "public"."fn_is_platform_admin"())) WITH CHECK (("public"."fn_role_at_least"("organization_id", 'admin'::"text") OR "public"."fn_is_platform_admin"()));
...
CREATE POLICY "tenant_isolation_ai_provider_credentials_modify" ON "public"."ai_provider_credentials" USING (("public"."fn_role_at_least"("organization_id", 'admin'::"text") OR "public"."fn_is_platform_admin"())) WITH CHECK (("public"."fn_role_at_least"("organization_id", 'admin'::"text") OR "public"."fn_is_platform_admin"()));



CREATE POLICY "tenant_isolation_ai_provider_credentials_select" ON "public"."ai_provider_credentials" FOR SELECT USING (("public"."fn_role_at_least"("organization_id", 'admin'::"text") OR "public"."fn_is_platform_admin"()));
...
-- ---- 0138: estatísticas de follow-up para o dashboard ----
create or replace function public.get_followup_enrollment_stats(p_org_id uuid)
returns table(total_enrollments bigint, active_enrollments bigint, converted_enrollments bigint,
  replied_enrollments bigint, exhausted_enrollments bigint, success_rate numeric,
  silence_sweep_rate numeric, daily_outcomes jsonb)
language sql security definer stable set search_path = public
as $$
with enrollment_summary as (
  select count(*) total,
    count(*) filter (where status = 'active') active_count,
    count(*) filter (where outcome = 'converted') converted_count,
    count(*) filter (where outcome = 'replied') replied_count,
    count(*) filter (where outcome = 'exhausted') exhausted_count
  from public.followup_enrollments
  where organization_id = p_org_id and created_at > now() - interval '30 days'
), daily_stats as (
  select date(created_at at time zone 'UTC')::text date_str,
    count(*) filter (where outcome = 'converted') converted,
    count(*) filter (where outcome = 'replied') replied,
    count(*) filter (where outcome = 'exhausted') exhausted
  from public.followup_enrollments
  where organization_id = p_org_id and created_at > now() - interval '7 days'
  group by date(created_at at time zone 'UTC') order by date_str desc limit 7
)
select coalesce(es.total,0)::bigint, coalesce(es.active_count,0)::bigint,
  coalesce(es.converted_count,0)::bigint, coalesce(es.replied_count,0)::bigint,
  coalesce(es.exhausted_count,0)::bigint,
  case when coalesce(es.total,0)=0 then 0 else (es.converted_count+es.replied_count)::numeric/es.total end,
  case when coalesce(es.active_count,0)=0 then 0 else es.active_count::numeric/es.total end,
  coalesce((select jsonb_agg(jsonb_build_object('date', d.date_str, 'converted', d.converted,
    'replied', d.replied, 'exhausted', d.exhausted) order by d.date_str) from daily_stats d), '[]'::jsonb)
...
-- ---- 0139: memória multi-turn do agente (tenant-scoped, append-only onde aplicável) ----
create table if not exists public.agent_memory (
  id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade,
  agent_id uuid not null references public.ai_agents(id) on delete cascade, contact_id uuid not null references public.contacts(id) on delete cascade,
  memory_type text not null check (memory_type in ('conversation','decision','preference','issue','resolution')),
  content text not null, context jsonb default '{}', expires_at timestamptz, created_at timestamptz not null default now(),
...
create policy agent_memory_org_isolation on public.agent_memory for all using (organization_id in (select fn_user_org_ids())) with check (organization_id in (select fn_user_org_ids()));

create table if not exists public.agent_decisions (
  id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade,
  agent_id uuid not null references public.ai_agents(id) on delete cascade, contact_id uuid not null references public.contacts(id) on delete cascade,
  decision text not null, reason text, outcome text check (outcome in ('success','partial','rejected','pending','unknown')),
...
create policy agent_decisions_org_isolation on public.agent_decisions for all using (organization_id in (select fn_user_org_ids())) with check (organization_id in (select fn_user_org_ids()));
...
create table if not exists public.agent_errors (
  ...
  message text not null, recovery_taken text, severity text check (severity in ('warning','error','critical')), created_at timestamptz not null default now()
);
...
create policy agent_errors_org_isolation on public.agent_errors for all using (organization_id in (select fn_user_org_ids())) with check (organization_id in (select fn_user_org_ids()));

create or replace view public.agent_context_view as
select organization_id, agent_id, contact_id,
  jsonb_agg(jsonb_build_object('id', id, 'type', memory_type, 'content', content, 'created_at', created_at) order by created_at desc) filter (where expires_at is null or expires_at > now()) memories,
...
create table if not exists public.ai_agent_command_approvals (
  ...
  reason text not null,
  status text not null default 'pending' check (status in ('pending','approved','denied','executing','executed','expired','failed')),
  expires_at timestamptz not null,
  decided_at timestamptz,
...
create index if not exists ai_agent_command_approvals_pending_idx on public.ai_agent_command_approvals (organization_id, expires_at) where status = 'pending';
create or replace function public.fn_ai_agent_command_approval_tenant_guard() returns trigger language plpgsql set search_path = public as $$ begin if not exists (select 1 from public.ai_agents a join public.ai_agent_versions v on v.id = new.agent_version_id and v.agent_id = a.id and v.organization_id = a.organization_id where a.id = new.agent_id and a.organization_id = new.organization_id) then raise exception 'agent_command_approval_tenant_mismatch' using errcode = '23503'; end if; return new; end; $$;
drop trigger if exists trg_ai_agent_command_approval_tenant_guard on public.ai_agent_command_approvals;
create trigger trg_ai_agent_command_approval_tenant_guard before insert or update of organization_id, agent_id, agent_version_id on public.ai_agent_command_approvals for each row execute function public.fn_ai_agent_command_approval_tenant_guard();
alter table public.ai_agent_command_approvals enable row level security;
drop policy if exists ai_agent_command_approvals_manager_all on public.ai_agent_command_approvals;
create policy ai_agent_command_approvals_manager_all on public.ai_agent_command_approvals for all to authenticated using (organization_id in (select public.fn_user_org_ids()) and public.fn_role_at_least(organization_id, 'manager')) with check (organization_id in (select public.fn_user_org_ids()) and public.fn_role_at_least(organization_id, 'manager'));
...
-- ---- 0156: arquivamento de conversa por utilizador -----------------------
...
create index if not exists conversation_archives_user_idx on public.conversation_archives (organization_id, user_id, archived_at desc);
...
create policy conversation_archives_select on public.conversation_archives for select to authenticated using (user_id = auth.uid() and organization_id in (select public.fn_user_org_ids()));
drop policy if exists conversation_archives_insert on public.conversation_archives;
create policy conversation_archives_insert on public.conversation_archives for insert to authenticated with check (user_id = auth.uid() and organization_id in (select public.fn_user_org_ids()));
drop policy if exists conversation_archives_delete on public.conversation_archives;
create policy conversation_archives_delete on public.conversation_archives for delete to authenticated using (user_id = auth.uid() and organization_id in (select public.fn_user_org_ids()));
...
begin
  ...
  if parent_org is null or parent_org <> NEW.organization_id then
    raise exception 'content_os tenant mismatch: %.%', TG_TABLE_NAME, TG_ARGV[1] using errcode = '23514';
  end if;
  return NEW;
end $$;

drop trigger if exists content_signals_source_tenant on public.content_signals; create trigger content_signals_source_tenant before insert or update on public.content_signals for each row execute function public.content_os_enforce_tenant_fk('content_sources', 'source_id');
drop trigger if exists competitor_monitors_competitor_tenant on public.competitor_monitors; create trigger competitor_monitors_competitor_tenant before insert or update on public.competitor_monitors for each row execute function public.content_os_enforce_tenant_fk('competitors', 'competitor_id');
drop trigger if exists competitor_events_competitor_tenant on public.competitor_events; create trigger competitor_events_competitor_tenant before insert or update on public.competitor_events for each row execute function public.content_os_enforce_tenant_fk('competitors', 'competitor_id');
drop trigger if exists competitor_events_monitor_tenant on public.competitor_events; create trigger competitor_events_monitor_tenant before insert or update on public.competitor_events for each row execute function public.content_os_enforce_tenant_fk('competitor_monitors', 'monitor_id');
drop trigger if exists content_opportunities_signal_tenant on public.content_opportunities; create trigger content_opportunities_signal_tenant before insert or update on public.content_opportunities for each row execute function public.content_os_enforce_tenant_fk('content_signals', 'signal_id');
drop trigger if exists content_opportunities_event_tenant on public.content_opportunities; create trigger content_opportunities_event_tenant before insert or update on public.content_opportunities for each row execute function public.content_os_enforce_tenant_fk('competitor_events', 'competitor_event_id');
drop trigger if exists content_ideas_campaign_tenant on public.content_ideas; create trigger content_ideas_campaign_tenant before insert or update on public.content_ideas for each row execute function public.content_os_enforce_tenant_fk('content_campaigns', 'campaign_id');
drop trigger if exists content_ideas_opportunity_tenant on public.content_ideas; create trigger content_ideas_opportunity_tenant before insert or update on public.content_ideas for each row execute function public.content_os_enforce_tenant_fk('content_opportunities', 'opportunity_id');
...
-- Achado numa auditoria de performance (2026-08-21): sem índice tocando
-- organization_id, a política de RLS (organization_id in (select
-- fn_user_org_ids())) força sequential scan nessas tabelas. Puramente
-- aditivo e idempotente — sem backfill, sem risco. Detalhe completo na
-- migration 20260821172926_0123_index_missing_organization_id.sql.
...
-- ---- 0116/0122: AI Platform foundation (snapshot final) ------------------
-- Estas tabelas são definidas originalmente em 0116 e a tabela de flags é
-- re-assentada por 0122 para o vocabulário final de features. O baseline precisa
-- conter o estado final para instalações fresh, sem depender da cadeia histórica
-- de migrations.
...
create index if not exists ai_projection_ledger_retry_idx
  on public.ai_projection_ledger (next_attempt_at)
...
-- ---- Content OS editorial persistence (migration 0158) ----
create table if not exists public.content_research_runs (id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade, opportunity_id uuid references public.content_opportunities(id) on delete set null, content_item_id uuid references public.content_items(id) on delete set null, provider text not null, query text not null, idempotency_key text not null, status text not null default 'queued' check (status in ('queued','running','succeeded','failed','cancelled')), attempt_count integer not null default 0 check (attempt_count >= 0), result jsonb not null default '{}'::jsonb, error_code text, started_at timestamptz, completed_at timestamptz, created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique (organization_id,idempotency_key), check (completed_at is null or started_at is not null), check (status not in ('succeeded','failed','cancelled') or completed_at is not null));
create table if not exists public.content_evidence (id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade, research_run_id uuid not null references public.content_research_runs(id) on delete cascade, source_id uuid references public.content_sources(id) on delete set null, source_url text not null, source_type text not null, publisher text, title text, excerpt text, locator text, content_hash text not null, authority_score numeric(5,2) check (authority_score between 0 and 100), verification_status text not null default 'unreviewed' check (verification_status in ('unreviewed','verified','disputed','rejected')), published_at timestamptz, retrieved_at timestamptz not null default now(), metadata jsonb not null default '{}'::jsonb, created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique (organization_id,research_run_id,source_url,content_hash));
create table if not exists public.content_claims (id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade, research_run_id uuid not null references public.content_research_runs(id) on delete cascade, claim_key text not null, claim_text text not null, claim_type text not null default 'factual' check (claim_type in ('factual','interpretation','allegation','opinion')), verification_status text not null default 'open' check (verification_status in ('open','confirmed','partially_confirmed','unconfirmed','conflicted','rejected')), confidence numeric(5,2) check (confidence between 0 and 100), is_critical boolean not null default false, checked_at timestamptz, notes text, created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique (organization_id,research_run_id,claim_key), check (verification_status = 'open' or checked_at is not null));
create table if not exists public.content_claim_evidence (id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade, claim_id uuid not null references public.content_claims(id) on delete cascade, evidence_id uuid not null references public.content_evidence(id) on delete cascade, relationship text not null check (relationship in ('supports','contradicts','contextualizes')), note text, created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique (organization_id,claim_id,evidence_id,relationship));
create table if not exists public.content_quality_gates (id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade, content_item_id uuid not null references public.content_items(id) on delete cascade, research_run_id uuid references public.content_research_runs(id) on delete set null, gate_type text not null check (gate_type in ('research','fact_check','originality','seo','editorial','safety','publish')), status text not null default 'pending' check (status in ('pending','passed','failed','waived')), score numeric(5,2) check (score between 0 and 100), findings jsonb not null default '{}'::jsonb, checked_by text, checked_at timestamptz, created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique (organization_id,content_item_id,gate_type), check (status = 'pending' or checked_at is not null));
create table if not exists public.content_revisions (id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade, content_item_id uuid not null references public.content_items(id) on delete cascade, research_run_id uuid references public.content_research_runs(id) on delete set null, revision_number integer not null check (revision_number > 0), body jsonb not null default '{}'::jsonb, change_summary text, status text not null default 'draft' check (status in ('draft','approved','published','superseded')), created_by text, created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique (organization_id,content_item_id,revision_number));
...
do $$ declare table_name text; begin foreach table_name in array array['content_research_runs','content_evidence','content_claims','content_claim_evidence','content_quality_gates','content_revisions'] loop execute format('alter table public.%I enable row level security',table_name); execute format('drop policy if exists tenant_isolation_%s_all on public.%I',table_name,table_name); execute format('create policy tenant_isolation_%s_all on public.%I for all to authenticated using ((organization_id in (select public.fn_user_org_ids())) or public.fn_is_platform_admin()) with check ((organization_id in (select public.fn_user_org_ids())) or public.fn_is_platform_admin())',table_name,table_name); execute format('drop trigger if exists %I on public.%I','trg_'||table_name||'_touch',table_name); execute format('create trigger %I before update on public.%I for each row execute function public.fn_touch_updated_at()','trg_'||table_name||'_touch',table_name); end loop; end $$;
...
notify pgrst, 'reload schema';
-- Hermes Unified Learning OS baseline reconciliation (0163).
create table if not exists public.hermes_research_experiments (
  id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade,
  subject_kind text not null, subject_id text not null, context_fingerprint text not null, goal text not null, strategy text not null,
  metric_name text, baseline_value double precision, observed_value double precision, score double precision,
  status text not null check (status in ('keep','discard','crash','inconclusive')),
  evidence_refs jsonb not null default '[]'::jsonb, metadata jsonb not null default '{}'::jsonb, source_version text,
  supersedes_id uuid references public.hermes_research_experiments(id), created_at timestamptz not null default now()
);
create index if not exists hermes_research_experiments_org_subject_idx on public.hermes_research_experiments (organization_id, subject_kind, subject_id, created_at desc);
create index if not exists hermes_research_experiments_org_fingerprint_idx on public.hermes_research_experiments (organization_id, context_fingerprint, created_at desc);
...
create policy tenant_isolation_hermes_research_experiments_all on public.hermes_research_experiments for all using (organization_id in (select * from public.fn_user_org_ids())) with check (organization_id in (select * from public.fn_user_org_ids()));

create table if not exists public.hermes_outcomes (
  id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade,
  run_id text not null, mission_id text, candidate_id uuid, subject_kind text not null, subject_id text not null,
  technical_quality double precision check (technical_quality is null or (technical_quality >= 0 and technical_quality <= 1)),
  cost_cents integer check (cost_cents is null or cost_cents >= 0), latency_ms integer check (latency_ms is null or latency_ms >= 0),
  kpi_name text, kpi_baseline double precision, kpi_observed double precision, evidence_refs jsonb not null default '[]'::jsonb,
  observed_at timestamptz not null, created_at timestamptz not null default now()
);
create index if not exists hermes_outcomes_org_run_idx on public.hermes_outcomes (organization_id, run_id, observed_at desc);
create index if not exists hermes_outcomes_org_subject_idx on public.hermes_outcomes (organization_id, subject_kind, subject_id, observed_at desc);
...
create policy tenant_isolation_hermes_outcomes_all on public.hermes_outcomes for all using (organization_id in (select * from public.fn_user_org_ids())) with check (organization_id in (select * from public.fn_user_org_ids()));

create table if not exists public.hermes_capability_identities (
  id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade,
  capability_kind text not null, canonical_identity text not null, immutable_revision text, content_fingerprint text not null,
  permission_fingerprint text not null, trust_status text not null check (trust_status in ('unknown','inspected','trusted','rejected','stale')),
  evidence_refs jsonb not null default '[]'::jsonb, inspected_at timestamptz, created_at timestamptz not null default now(),
  unique (organization_id, capability_kind, canonical_identity, content_fingerprint, permission_fingerprint)
);
create index if not exists hermes_capability_identities_org_identity_idx on public.hermes_capability_identities (organization_id, capability_kind, canonical_identity, created_at desc);
...
-- ---- F1 identity mapping: infrastructure only; dual-read remains disabled ----
...
create table if not exists public.identity_user_mappings (
  ...
  legacy_auth_user_id uuid, email_snapshot citext,
  status text not null default 'pending' check (status in ('active','pending','blocked','retired')),
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(), migrated_at timestamptz,
  constraint identity_user_mappings_firebase_uid_key unique (firebase_uid), constraint identity_user_mappings_legacy_auth_user_id_key unique (legacy_auth_user_id)
);
create index if not exists identity_user_mappings_firebase_uid_idx on public.identity_user_mappings (firebase_uid);
create index if not exists identity_user_mappings_legacy_auth_user_id_idx on public.identity_user_mappings (legacy_auth_user_id);
create table if not exists public.identity_user_mapping_audit (
  id uuid primary key default gen_random_uuid(), mapping_id uuid references public.identity_user_mappings(id) on delete set null,
  firebase_uid text not null, legacy_auth_user_id uuid, action text not null check (action in ('dry_run_matched','dry_run_pending','created','unchanged','collision')),
  details jsonb not null default '{}'::jsonb, created_at timestamptz not null default now()
);
create index if not exists identity_user_mapping_audit_firebase_uid_idx on public.identity_user_mapping_audit (firebase_uid, created_at desc);
create or replace function public.fn_identity_user_mapping_audit_immutable() returns trigger language plpgsql set search_path = public, pg_temp as $$ begin raise exception 'identity_user_mapping_audit is append-only'; end; $$;
drop trigger if exists identity_user_mapping_audit_immutable on public.identity_user_mapping_audit;
create trigger identity_user_mapping_audit_immutable before update or delete on public.identity_user_mapping_audit for each row execute function public.fn_identity_user_mapping_audit_immutable();
create table if not exists public.identity_migration_flags (flag_name text primary key, enabled boolean not null default false, updated_at timestamptz not null default now(), constraint identity_migration_flags_known_flag check (flag_name in ('firebase_dual_read')));
insert into public.identity_migration_flags (flag_name, enabled) values ('firebase_dual_read', false) on conflict (flag_name) do nothing;
create or replace function public.fn_firebase_dual_read_enabled() returns boolean language sql stable set search_path = public, pg_temp as $$ select coalesce((select enabled from public.identity_migration_flags where flag_name = 'firebase_dual_read'), false); $$;
create or replace function public.backfill_identity_user_mappings(p_identities jsonb, p_dry_run boolean default true)
returns table (total_input bigint, matched bigint, pending bigint, duplicate_firebase_uid bigint, duplicate_legacy_auth_user_id bigint, email_collision bigint, deleted_user bigint, missing_membership bigint, persisted bigint)
language plpgsql security definer set search_path = public, pg_temp as $$
declare item jsonb; firebase_uid_value text; legacy_auth_user_id_value uuid; email_value citext; match_status text; collision_value text; mapping_id_value uuid;
total_value bigint := 0; matched_value bigint := 0; pending_value bigint := 0; duplicate_firebase_value bigint := 0; duplicate_legacy_value bigint := 0; email_collision_value bigint := 0; deleted_user_value bigint := 0; missing_membership_value bigint := 0; persisted_value bigint := 0;
begin
  if jsonb_typeof(p_identities) <> 'array' then raise exception 'p_identities must be a JSON array'; end if;
  for item in select value from jsonb_array_elements(p_identities) loop
    total_value := total_value + 1; firebase_uid_value := nullif(item->>'firebase_uid',''); match_status := coalesce(item->>'match_status','pending'); collision_value := item->>'collision'; email_value := nullif(item->>'email_snapshot','');
    if firebase_uid_value is null then raise exception 'firebase_uid is required for every identity'; end if;
    legacy_auth_user_id_value := nullif(item->>'legacy_auth_user_id','')::uuid;
    if match_status = 'matched' and legacy_auth_user_id_value is not null then matched_value := matched_value + 1; else pending_value := pending_value + 1; end if;
    ...
    if match_status = 'matched' and legacy_auth_user_id_value is not null and collision_value is null then
      insert into public.identity_user_mappings (firebase_uid, legacy_auth_user_id, email_snapshot, status, migrated_at) values (firebase_uid_value, legacy_auth_user_id_value, email_value, 'active', now()) on conflict (firebase_uid) do update set email_snapshot = excluded.email_snapshot, updated_at = now() returning id into mapping_id_value;
      persisted_value := persisted_value + 1;
      insert into public.identity_user_mapping_audit (mapping_id, firebase_uid, legacy_auth_user_id, action, details) values (mapping_id_value, firebase_uid_value, legacy_auth_user_id_value, 'created', jsonb_build_object('source','backfill'));
    else
      insert into public.identity_user_mappings (firebase_uid, email_snapshot, status) values (firebase_uid_value, email_value, 'pending') on conflict (firebase_uid) do update set email_snapshot = excluded.email_snapshot, updated_at = now() returning id into mapping_id_value;
      persisted_value := persisted_value + 1;
      insert into public.identity_user_mapping_audit (mapping_id, firebase_uid, action, details) values (mapping_id_value, firebase_uid_value, 'collision', jsonb_build_object('source','backfill','collision',collision_value));
    end if;
  end loop;
  return query select total_value, matched_value, pending_value, duplicate_firebase_value, duplicate_legacy_value, email_collision_value, deleted_user_value, missing_membership_value, persisted_value;
end; $$;

...
-- ---- score de probabilidade com evidência (migration 0074) ----
-- Idempotente e auto-curativo: as colunas usam `if not exists`, o backfill roda
-- ANTES da constraint (um clone com score órfão de razão tem o SCORE apagado —
-- não a razão inventada), e as constraints são drop+add.
alter table public.crm_leads
...
alter table public.crm_leads drop constraint if exists crm_leads_score_needs_reason;
alter table public.crm_leads
...
alter table public.crm_leads drop constraint if exists crm_leads_score_range;
alter table public.crm_leads
  add constraint crm_leads_score_range check (
    ai_probability is null or (ai_probability >= 0 and ai_probability <= 100)
  );

alter table public.crm_leads drop constraint if exists crm_leads_score_band_check;
alter table public.crm_leads
...
comment on column public.crm_leads.ai_probability_evidence is
  'O QUE SUSTENTA (N referências), cada chave apontando para UMA tabela: activity_ids→crm_lead_activities, message_ids→messages, checkpoint_ids→lead_checkpoints. Constraint exige pelo menos uma.';

comment on column public.crm_leads.ai_probability_band is
  'Faixa exibida (frio/morno/quente). Persistida porque HISTERESE precisa da faixa anterior: sem ela, score oscilando no limiar faz o card piscar a cada recálculo.';

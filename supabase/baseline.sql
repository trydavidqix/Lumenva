...
-- 0200_f2_tenant_isolation: runtime roles, Firebase identity mapping and tenant RLS probe.
do $$
begin
...
create table if not exists public.identity_user_mappings (
  id uuid primary key default gen_random_uuid(), firebase_uid text not null unique,
  user_id uuid not null, active boolean not null default true,
...
create table if not exists public.tenant_access_audit (
  ...
  organization_id uuid not null, request_id text not null, action text not null,
  created_at timestamptz not null default now()
);
...
returns uuid language sql security definer set search_path = public, pg_temp stable as $$
  select user_id from public.identity_user_mappings
   where firebase_uid = p_firebase_uid and active limit 1
$$;
...
create or replace function public.record_platform_admin_tenant_access(p_organization_id uuid, p_request_id text)
returns uuid language plpgsql security definer set search_path = public, pg_temp as $$
declare v_user_id uuid := nullif(current_setting('app.user_id', true), '')::uuid; v_audit_id uuid;
begin
  if current_setting('app.is_platform_admin', true) <> 'true' then raise exception 'platform admin context required'; end if;
  insert into public.tenant_access_audit(user_id, organization_id, request_id, action)
  values (v_user_id, p_organization_id, p_request_id, 'platform_admin.tenant_access') returning id into v_audit_id;
  return v_audit_id;
end
$$;
...
begin
  foreach role_name in array array['app_runtime', 'worker_runtime', 'platform_admin_runtime'] loop
    ...
    execute format($policy$create policy f2_%1$s_select on public.f2_tenant_isolation_probe for select to %1$s using (organization_id::text = current_setting('app.organization_id', true))$policy$, role_name);
    execute format($policy$create policy f2_%1$s_insert on public.f2_tenant_isolation_probe for insert to %1$s with check (organization_id::text = current_setting('app.organization_id', true))$policy$, role_name);
    execute format($policy$create policy f2_%1$s_update on public.f2_tenant_isolation_probe for update to %1$s using (organization_id::text = current_setting('app.organization_id', true)) with check (organization_id::text = current_setting('app.organization_id', true))$policy$, role_name);
    execute format($policy$create policy f2_%1$s_delete on public.f2_tenant_isolation_probe for delete to %1$s using (organization_id::text = current_setting('app.organization_id', true))$policy$, role_name);
  end loop;
end
$$;
...
-- platform_admin_runtime writes audit only through SECURITY DEFINER; no direct table grant.
...
-- 0202_f1_identity_mapping_v2: F1 forward-fix on top of F2's canonical mapping.
...
create policy identity_user_mapping_candidates_migration_select
  on public.identity_user_mapping_candidates for select to migration_admin using (true);
...
create policy identity_user_mapping_audit_migration_select
  on public.identity_user_mapping_audit for select to migration_admin using (true);
...
create policy identity_migration_flags_migration_select
  on public.identity_migration_flags for select to migration_admin using (true);
...
returns uuid language sql security definer set search_path = public, pg_temp stable as $$
  select user_id from public.identity_user_mappings
   where firebase_uid = p_firebase_uid and active and retired_at is null limit 1
$$;
revoke execute on function public.resolve_firebase_identity(text) from public, anon, authenticated;
...
returns boolean language sql security definer set search_path = public, pg_temp stable as $$
  select coalesce((select enabled from public.identity_migration_flags
    where flag_name = 'firebase_dual_read'), false);
$$;
...
create or replace function public.backfill_identity_user_mappings(
  p_identities jsonb, p_run_id uuid, p_dry_run boolean default true
)
returns table (
  total_input bigint, matched bigint, pending bigint,
  duplicate_firebase_uid bigint, duplicate_legacy_auth_user_id bigint,
  email_collision bigint, deleted_user bigint, missing_membership bigint,
  persisted bigint
)
language plpgsql security definer set search_path = public, pg_temp as $$
declare
  item jsonb; firebase_uid_value text; proposed_user_id_value uuid;
  email_value citext; match_status_value text; collision_value text;
  existing_mapping_id uuid; existing_user_id uuid; candidate_id_value uuid;
  action_value text; total_value bigint := 0; matched_value bigint := 0;
  pending_value bigint := 0; duplicate_firebase_value bigint := 0;
  duplicate_legacy_value bigint := 0; email_collision_value bigint := 0;
  deleted_user_value bigint := 0; missing_membership_value bigint := 0;
...
begin
  ...
  for item in select value from jsonb_array_elements(p_identities) loop
    ...
    if match_status_value = 'matched' and proposed_user_id_value is not null
       and collision_value is null then matched_value := matched_value + 1;
    else pending_value := pending_value + 1;
    end if;
    if collision_value = 'duplicate_firebase_uid' then duplicate_firebase_value := duplicate_firebase_value + 1;
    elsif collision_value = 'duplicate_legacy_auth_user_id' then duplicate_legacy_value := duplicate_legacy_value + 1;
    elsif collision_value = 'email_collision' then email_collision_value := email_collision_value + 1;
    elsif collision_value = 'deleted_user' then deleted_user_value := deleted_user_value + 1;
    elsif collision_value = 'missing_membership' then missing_membership_value := missing_membership_value + 1;
    end if;
    if p_dry_run then continue; end if;
    existing_mapping_id := null; existing_user_id := null;
    select id, user_id into existing_mapping_id, existing_user_id
      from public.identity_user_mappings where firebase_uid = firebase_uid_value;
    if match_status_value = 'matched' and proposed_user_id_value is not null
      ...
      if existing_mapping_id is null then
        ...
        values (firebase_uid_value, proposed_user_id_value, true, email_value, now(), now())
        ...
        update public.identity_user_mappings
           set email_snapshot = coalesce(email_value, email_snapshot),
               updated_at = now(), migrated_at = coalesce(migrated_at, now())
      ...
      values (p_run_id, existing_mapping_id, firebase_uid_value, action_value,
        jsonb_build_object('source', 'backfill'))
      on conflict (run_id, firebase_uid, action) do nothing;
    else
      if existing_mapping_id is not null and proposed_user_id_value is not null
      ...
      values (firebase_uid_value, proposed_user_id_value, email_value,
      ...
      on conflict (firebase_uid) do update set
        ...
        status = excluded.status, collision_code = excluded.collision_code,
      ...
      values (p_run_id, candidate_id_value, firebase_uid_value, action_value,

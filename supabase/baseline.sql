...
create table if not exists public.ai_projection_ledger (
  ...
  source_id text not null, source_version text not null, idempotency_key text not null,
  status text not null default 'pending' check (status in ('pending','processing','applied','failed','deleted')), attempts integer not null default 0 check (attempts >= 0),
  next_attempt_at timestamptz, last_error_code text, last_error_at timestamptz, applied_at timestamptz,
...
create policy tenant_isolation_ai_platform_feature_flags_all on public.ai_platform_feature_flags for all to authenticated using (organization_id in (select public.fn_user_org_ids())) with check (organization_id in (select public.fn_user_org_ids()));
drop policy if exists tenant_isolation_ai_projection_ledger_all on public.ai_projection_ledger;
create policy tenant_isolation_ai_projection_ledger_all on public.ai_projection_ledger for all to authenticated using (organization_id in (select public.fn_user_org_ids())) with check (organization_id in (select public.fn_user_org_ids()));

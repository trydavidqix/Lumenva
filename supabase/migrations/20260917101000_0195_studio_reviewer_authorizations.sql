...
CREATE POLICY studio_reviewer_authorizations_tenant_all ON public.studio_reviewer_authorizations FOR ALL TO authenticated USING (organization_id IN (SELECT public.fn_user_org_ids())) WITH CHECK (organization_id IN (SELECT public.fn_user_org_ids()));

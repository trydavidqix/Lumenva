...
describe("Step 3 — retry does not duplicate the mock side effect", () => {
  ...
  it("an invalid signature is rejected before the dedupe store is touched and the mock side effect never fires", async () => {
    ...
    expect(sim.sideEffectCalls, "REDACTED_SECRET signature must never reach the side effect").toBe(0);
    expect(sim.claimedKeys.size, "REDACTED_SECRET signature must never claim an idempotency key").toBe(0);

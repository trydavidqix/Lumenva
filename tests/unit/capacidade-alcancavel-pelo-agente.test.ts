...
/**
 * DÍVIDA MEDIDA em `99cd0fc`, antes desta wave: quatro capacidades de escrita já
 * publicadas exigem `manager` e nenhum agente as alcança.
 *
 * Não são "quase certo" nem "provavelmente": foram lidas uma a uma em
 * `lib/mcp/tools/`. Estão aqui porque consertá-las é decisão do épico, não desta
 * wave — subir o papel do token efêmero é mexer no modelo de permissão, que o
 * despacho da W4 põe explicitamente fora de escopo. Ver BUG-02 no
 * `HANDOFF-ia-360.md`.
 *

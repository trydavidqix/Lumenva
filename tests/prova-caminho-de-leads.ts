...
async function rodada(i: number, lead: { id: string; title: string }): Promise<string> {
  ...
  const chegou = { leads: 0, atividades: 0 };
  const c1 = cli
  ...
  if (e1 !== "SUBSCRIBED" || e2 !== "SUBSCRIBED") return `assinatura falhou (${e1}/${e2})`;
  ...
  return `crm_leads UPDATE: ${chegou.leads > 0 ? "CHEGOU" : "sumiu"} · crm_lead_activities INSERT: ${chegou.atividades > 0 ? "CHEGOU" : "sumiu"}`;
}
...
async function main(): Promise<void> {
  ...
  const leadsOk = linhas.filter((l) => l.includes("crm_leads UPDATE: CHEGOU")).length;
  const ativOk = linhas.filter((l) => l.includes("INSERT: CHEGOU")).length;
  console.info(
    `\n=== ${leadsOk}/3 UPDATE de crm_leads · ${ativOk}/3 INSERT de crm_lead_activities ===\n` +
      (ativOk === 3 && leadsOk === 0

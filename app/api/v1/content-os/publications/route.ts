...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  } catch (error) { const code = error instanceof Error && "code" in error ? String((error as { code?: unknown }).code) : "internal_error"; if (code === "idempotency_conflict") return fail(code, "Idempotency-Key já foi usada com outro payload.", 409, { requestId }); if (code === "publish_quality_gate_required" || code === "publication_not_publishable") return fail(code, "A publicação não está pronta para ser publicada.", 422, { requestId }); return fail("internal_error", "Não foi possível criar a publicação.", 500, { requestId }); }
}

/** GET proposals for one agent. Legacy rows remain compatible; Phase 6 rows are normalized and tenant/agent scoped. */
...
export async function GET(req: NextRequest, ctx: Ctx): Promise<Response> {
  ...
  const { data: agent } = await admin
    ...
    .maybeSingle();
  if (!agent) return fail("not_found", "Agente não encontrado nesta organização.", 404, { requestId });
  ...
  if (error) return fail("internal_error", "Falha ao carregar as propostas.", 500, { requestId });

  const items = (data ?? []).flatMap((row) => {
    const phase6 = mapPhase6ProposalRow(row as never);
    if (phase6 && phase6.scope.agentId !== id) return [];
    return [{ ...row, phase6 }];
  });

...
export async function GET(request: Request) {
  try {
    ...
    cookieStore.set('fb_oauth_state', state, {
      ...
      sameSite: 'lax',
      path: '/api/integrations/meta/facebook/callback',
      maxAge: 10 * 60, // 10 minutes
    })
    cookieStore.set('fb_oauth_workspace', owner.workspaceId, {
      httpOnly: true,
      secure: process.env.NODE_ENV === 'production',
      sameSite: 'lax',
      path: '/api/integrations/meta/facebook/callback',
    ...
    if (!clientId) throw new Error('FACEBOOK_CLIENT_ID is not set')
    
    const origin = new URL(request.url).origin
    const redirectUri = `${origin}/api/integrations/meta/facebook/callback`

...
const rowToReceipt = (row: ReceiptRow): DeliveryReceipt => { const { tenant_id: _tenantId, ...receipt } = row; return Object.freeze({ ...receipt, artifact_refs: freeze(row.artifact_refs), evidence_refs: freeze(row.evidence_refs) }); };

export class DeliveryReceiptStore {
  constructor(private readonly db: Queryable) {}
  async get(tenantId: string, receiptId: string): Promise<DeliveryReceipt | undefined> {
    const result = await this.db.query<ReceiptRow>("select * from public.delivery_receipts where tenant_id=$1 and delivery_receipt_id=$2", [tenantId, receiptId]);
  ...
  async findTenantByReceiptId(receiptId: string): Promise<string | undefined> {
    const result = await this.db.query<{ tenant_id: string }>("select tenant_id from public.delivery_receipts where delivery_receipt_id=$1", [receiptId]);
...
export function getDeliveryReceipt(store: DeliveryReceiptStore, organizationId: string, receiptId: string): Promise<DeliveryReceipt | undefined> { return store.get(organizationId, receiptId); }

export async function createDeliveryReceipt(
  plan: DeliveryPlan, artifact: DeliveryArtifact, buildEvidence: DeliveryBuildEvidence, stateStore: BuildPlanStateStore, receiptStore: DeliveryReceiptStore,

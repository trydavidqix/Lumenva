...
- Migrations: `infra/supabase/migrations/<YYYYMMDDHHMMSS>_0050_agent_harness.sql`, idempotente,
  sem BEGIN/COMMIT, RLS `tenant_isolation_<tabela>_all` via `fn_user_org_ids()` em toda tabela
  org-scoped, + apêndice em `infra/supabase/baseline.sql` + linha no `MANIFEST.md`.

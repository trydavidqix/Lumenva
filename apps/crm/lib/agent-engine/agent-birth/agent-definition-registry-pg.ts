...
export class PostgresAgentDefinitionRegistry {
  ...
  async register(input: DurableRegistryInput): Promise<void> {
    const authority = await input.authorityStore.verify({
      ...
      definitionId: typeof input.definition === "object" && input.definition !== null && "id" in input.definition ? String((input.definition as any).id) : "",
      definitionVersion: typeof input.definition === "object" && input.definition !== null && "version" in input.definition ? String((input.definition as any).version) : "",
    ...
    const result = await this.db.query(`INSERT INTO public.agent_definition_registry
  ...
  async get(tenantId: string, id: string, version: string): Promise<AgentDefinition | null> {
    const result = await this.db.query<any>(`SELECT definition_id,definition_version,identity,mission,boundaries,authority,escalation,status FROM public.agent_definition_registry WHERE organization_id=$1 AND definition_id=$2 AND definition_version=$3`, [tenantId,id,version]);
    const row=result.rows[0]; if (!row) return null;
    return { id: row.definition_id, version: row.definition_version, identity: row.identity, mission: row.mission, boundaries: Array.isArray(row.boundaries) ? [...row.boundaries] : JSON.parse(row.boundaries), authority: row.authority, escalation: row.escalation, status: row.status };
  }
}

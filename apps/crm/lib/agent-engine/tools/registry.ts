...
const INTERNAL_TOOL_POLICIES: Readonly<Record<string, InternalToolPolicy>> = {
  ...
  schedule_followup: {
    owner: 'agent-engine.followup',
    risk: 'r1_reversible_write',
    timeoutMs: 15_000,
    maxRetries: 1,
  },
  save_lead_note: {
    owner: 'agent-engine.memory',
    risk: 'r1_reversible_write',
    timeoutMs: 15_000,
    maxRetries: 1,
  },
  ...
  search_knowledge: {
    owner: 'agent-engine.knowledge',
    risk: 'r0_read',
    timeoutMs: 20_000,
    maxRetries: 1,
  },
  request_human_handoff: {
    owner: 'agent-engine.handoff',
    risk: 'r1_reversible_write',
    timeoutMs: 15_000,
    maxRetries: 1,
  },
  read_skill_reference: {
    owner: 'agent-engine.skills',
    risk: 'r0_read',
    timeoutMs: 10_000,
    maxRetries: 1,
  },
  open_human_case: {
    owner: 'agent-engine.cases',
    risk: 'r1_reversible_write',
    timeoutMs: 15_000,
    maxRetries: 1,
  },
...
function hasSideEffect(risk: AgentToolRisk): boolean {
  return risk !== 'r0_read';
}

function definitionFromPolicy(input: {
  id: string;
  owner: string;
  source: AgentToolSource;
  schema: unknown;
  ...
  return {
    ...
    owner: input.owner,
    source: input.source,
    schema: { kind: 'inline', value: input.schema },
    risk: input.risk,
    hasSideEffect: sideEffect,
    idempotencyRequired: sideEffect,
    timeoutMs: input.timeoutMs,
    maxRetries: input.maxRetries,
  };
}

export function createInternalToolDefinitions(
  toolDefs: InlineAgentToolShape,
): AgentToolDefinition[] {
  return Object.entries(toolDefs).map(([id, def]) => {
    const policy = INTERNAL_TOOL_POLICIES[id];
    if (!policy) {
      throw new Error(`missing_internal_tool_metadata:${id}`);
    }

    return definitionFromPolicy({
      id,
      owner: policy.owner,
      source: 'internal',
...
export function createMcpToolDefinitions(
  catalog: ReadonlyArray<McpToolCatalogEntry>,
): AgentToolDefinition[] {
  return catalog.map((entry) => {
    const risk = mcpRisk(entry);
    return definitionFromPolicy({
      id: entry.name,
      owner: `mcp.${entry.category}`,
      source: 'mcp',
      schema: { catalogEntry: entry.name },
      risk,
      timeoutMs: risk === 'r0_read' ? 15_000 : 30_000,
      maxRetries: risk === 'r4_destructive_admin' ? 0 : 1,
    });
  });
}
...
function assertValidDefinition(definition: AgentToolDefinition): void {
  const validRisk =
    definition.risk === 'r0_read' ||
    definition.risk === 'r1_reversible_write' ||
    definition.risk === 'r2_external_communication' ||
    definition.risk === 'r3_sensitive_commercial' ||
    definition.risk === 'r4_destructive_admin';

  const validBase =
    definition.id.trim().length > 0 &&
    definition.owner.trim().length > 0 &&
    (definition.source === 'internal' || definition.source === 'mcp') &&
    definition.schema.kind === 'inline' &&
    validRisk &&
    Number.isFinite(definition.timeoutMs) &&
    definition.timeoutMs > 0 &&
    Number.isInteger(definition.maxRetries) &&
    definition.maxRetries >= 0;

  const validRiskContract =
    definition.risk === 'r0_read'
      ? definition.hasSideEffect === false && definition.idempotencyRequired === false
      : definition.hasSideEffect === true && definition.idempotencyRequired === true;

  if (!validBase || !validRiskContract) {
    throw new Error(`invalid_tool_metadata:${definition.id}`);
  }
}
...
export function createToolRegistry(
  ...
  for (const definition of definitions) {
    ...
    if (registry.has(definition.id)) {
      throw new Error(`duplicate_tool_id:${definition.id}`);
    }

...
function memoryStore(): ApprovalStore {
  ...
  return { save: async (row) => void rows.set(row.id, row), load: async (id) => rows.get(id) ?? null, compareAndSet: async (id, expectedStatus, next) => { const current = rows.get(id); if (!current || current.status !== expectedStatus) return false; rows.set(id, next); return true; } };
}

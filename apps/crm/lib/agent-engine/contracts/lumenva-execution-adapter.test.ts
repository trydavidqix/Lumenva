...
import {
  ...
  createPostgresLumenvaExecutionPersistence,
...
function memoryPersistence(initial?: LumenvaExecutionRecord): {
  persistence: LumenvaExecutionPersistence;
  read: () => LumenvaExecutionRecord | null;
} {
...
describe('LumenvaExecutionAdapter', () => {
  it('inicia um run novo como running e persiste a identidade de execução', async () => {
    const memory = memoryPersistence();
    const adapter = new LumenvaExecutionAdapter(memory.persistence);
  ...
  it('reabre o checkpoint persistido após interrupção e não repete side effect concluído', async () => {
    const previous: LumenvaExecutionRecord = {
      ...identity,
      jobId: 'job-1',
      workerId: 'worker-old',
      status: 'running',
  ...
  it('persistência Postgres usa job_queue como checkpoint durável e event_log como trilha', async () => {
    ...
    const persistence = createPostgresLumenvaExecutionPersistence({ query } as never);
    const record: LumenvaExecutionRecord = {
      ...identity,
      jobId: 'job-1',
      workerId: 'worker-1',
      status: 'running',

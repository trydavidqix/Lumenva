...
function validateAction(worker: BrowserMeshWorker, action: ActionEnvelope): void {
  ...
  if (!/^P[0-4]$/.test(action.permission_level) || !/^R[0-4]$/.test(action.risk_level)) throw new Error("action_policy_invalid");
  if (Math.max(Number(action.permission_level.slice(1)), Number(action.risk_level.slice(1))) >= 2 && !action.approval_id) throw new Error("action_approval_required");
...
export class ActionBus {
  ...
  private async runOnce(worker: BrowserMeshWorker, action: ActionEnvelope, key: string, adapter: (action: ActionEnvelope) => Promise<unknown>): Promise<ActionResult> {
    const result = redact(await adapter(action));
    const evidence: ActionEvidence = { action_id: action.action_id, assignment_id: action.assignment_id, worker_id: worker.worker_id, action_type: action.action_type, result, persisted_at: new Date().toISOString() };
    ...
    this.evidence.set(key, structuredClone(evidence));

...
function validateAction(worker: BrowserMeshWorker, action: ActionEnvelope, approvals: readonly ApprovalRecord[]): void {
...
export class ActionBus {
  ...
  constructor(private readonly approvals: readonly ApprovalRecord[] = []) {}
  async execute(worker: BrowserMeshWorker, action: ActionEnvelope, adapter: (action: ActionEnvelope) => Promise<unknown>): Promise<ActionResult> {
    validateAction(worker, action, this.approvals);

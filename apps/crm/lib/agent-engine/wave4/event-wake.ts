...
export type WakeActor = { actor_id: string; organization_id: string; capabilities: readonly string[]; enabled?: boolean };
export class WakeActorRegistry {
  private readonly actors = new Map<string, WakeActor>();
  constructor(actors: readonly WakeActor[] = []) { for (const actor of actors) this.actors.set(`${actor.organization_id}:${actor.actor_id}`, { ...actor, capabilities: [...actor.capabilities] }); }
  get(organizationId: string, actorId: string): WakeActor | undefined { return this.actors.get(`${organizationId}:${actorId}`); }
}
...
export async function wakeEventPersisted(event: unknown, workers: readonly WakeWorker[], policy: WakePolicy, secret: string, store: PostgresWakeEventStore, actorRegistry: WakeActorRegistry): Promise<WakeDecision> {
  const decision = wakeEvent(event, workers, policy, secret, actorRegistry);
...
export async function dispatchWakeEvent(event: unknown, workers: readonly WakeWorker[], policy: WakePolicy, secret: string, store: PostgresWakeEventStore, actorRegistry: WakeActorRegistry, execute: (decision: Extract<WakeDecision, { status: "WAKED" }>) => Promise<void>): Promise<WakeDecision> {
  const decision = await wakeEventPersisted(event, workers, policy, secret, store, actorRegistry);
  if (decision.status === "WAKED") await execute(decision);
  return decision;
}

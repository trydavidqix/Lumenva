...
describe('searchKnowledge', () => {
  it('emite somente metadados de retrieval correlacionados ao job, sem a consulta ou o conteúdo dos hits', async () => {
    ...
    const tracer: AiTracer = {
      startSpan: vi.fn(async (input) => {
        starts.push(input);
        return { end: vi.fn(async (input) => { ends.push(input); }) };
      }),
    };

    await searchKnowledge(
      { query: vi.fn().mockResolvedValue({ rows: [hit] }) } as unknown as pg.Pool,
      {
        organizationId: 'org-1',
        kbVersionId: 'kb-1',
        query: 'Ana ana@example.test quer saber o frete',
        topK: 5,
        threshold: 0.72,
        jobId: 'job-9',
      },
      { embed: vi.fn().mockResolvedValue({ embedding: [0.1, 0.2] }), tracer } as never,
    );
    ...
    expect(ends).toEqual([{ metrics: { result_count: 1 } }]);
    expect(JSON.stringify({ starts, ends })).not.toContain('ana@example.test');
    expect(JSON.stringify({ starts, ends })).not.toContain('Frete grátis acima');
  });

  it('fecha a busca indisponível com um código redigido, sem o erro que pode conter PII', async () => {
    const ends: Parameters<AiTraceSpan['end']>[0][] = [];
    const tracer: AiTracer = {
      startSpan: vi.fn(async () => ({ end: vi.fn(async (input) => { ends.push(input); }) })),
    };

    const out = await searchKnowledge(
      { query: vi.fn() } as unknown as pg.Pool,
      { organizationId: 'org-1', kbVersionId: 'kb-1', query: 'frete', topK: 5, threshold: 0.72, jobId: 'job-9' },
      { embed: vi.fn().mockRejectedValue(new Error('provider refused ana@example.test')), tracer } as never,
    );

    expect(out).toMatchObject({ ok: false, error: { code: 'knowledge_unavailable' } });
    expect(ends).toEqual([{ error: 'knowledge_unavailable' }]);
    expect(JSON.stringify(ends)).not.toContain('ana@example.test');
  });

import { assertPublishableContent, type ContentProvenanceInput } from "@/lib/knowledge/content-provenance";
...
async function assertPublicationConsent(repository: PublishContentRepository, input: PublishContentInput): Promise<void> {
  ...
  for (const requirement of requirements) {
    ...
    if (consent.granted_at && Date.parse(consent.granted_at) > now) {
      throw new PublicationConsentError("Consent is not active yet.");
    }
    if (consent.revoked_at && Date.parse(consent.revoked_at) <= now) {
      throw new PublicationConsentError("Consent has been revoked.");
    }
    if (consent.retention_until && Date.parse(consent.retention_until) <= now) {
      throw new PublicationConsentError("Consent has expired.");
    }
  }
}

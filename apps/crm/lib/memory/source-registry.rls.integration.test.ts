  ...
  it("persiste via migration e isola tenants com role sem BYPASSRLS", async () => {
    ...
    try {
      ...
      for (let i = 0; i < 120; i += 1) {
        ...
        catch (error) { if (i === 119) throw error; await new Promise((resolve) => setTimeout(resolve, 200)); }
      }
      ...
      try {
        ...
        await admin.query(await readFile(`${root}/20260913000000_0163_hermes_source_registry.sql`, "utf8"));
        await admin.query(await readFile(`${root}/20260913010000_0164_hermes_source_registry_rls.sql`, "utf8"));

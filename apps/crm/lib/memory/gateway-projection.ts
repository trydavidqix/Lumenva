...
export type MemoryGateway = {
  ...
  read(backend: string, query: ProjectionQuery): Promise<readonly MemoryRecord[]>;
};

function validNamespace(namespace: string): boolean {
  return /^(owner|home|company):[^:]+$/.test(namespace);
}

function newer(a: MemoryRecord, b: MemoryRecord): MemoryRecord {
...
/** Replays all gateway backends into the current subject/scope projection. */
export async function rebuildProjectionViaGateway(
  ...
  for (const record of byId.values()) if (record.supersedes) superseded.add(record.supersedes);
  return [...byId.values()]

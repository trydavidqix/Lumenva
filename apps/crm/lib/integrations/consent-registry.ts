...
export class PostgresConsentRegistry {
  ...
  async revoke(organizationId: string, consentId: string, revokedAt = new Date().toISOString()): Promise<ConsentRecord> {
    if (!Number.isFinite(Date.parse(revokedAt))) throw new Error("consent_timestamp_invalid");
    const result = await this.db.query<StoredConsentRecord>(`update public.contact_consents set status='REVOKED',revoked_at=$3,updated_at=now() where organization_id=$1 and consent_id=$2 and status='GRANTED' returning consent_id,organization_id,subject_ref,purpose,channel,legal_basis_ref,status,granted_at,revoked_at,retention_until,source_refs,evidence_refs`, [organizationId, consentId, revokedAt]);
  ...
  async get(organizationId: string, consentId: string): Promise<ConsentRecord | undefined> {
    const result = await this.db.query<StoredConsentRecord>(`select consent_id,organization_id,subject_ref,purpose,channel,legal_basis_ref,status,granted_at,revoked_at,retention_until,source_refs,evidence_refs from public.contact_consents where organization_id=$1 and consent_id=$2`, [organizationId, consentId]);
  ...
  async canContact(organizationId: string, subjectRef: string, channel: ConsentChannel, purpose: string): Promise<boolean> {
    const result = await this.db.query<{ allowed: boolean }>(`select exists(select 1 from public.contact_consents where organization_id=$1 and subject_ref=$2 and channel=$3 and purpose=$4 and status='GRANTED' and granted_at <= now() and (retention_until is null or retention_until > now())) as allowed`, [organizationId, subjectRef, channel, purpose]);

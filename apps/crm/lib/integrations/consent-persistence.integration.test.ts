...
describe("durable consent enforcement against Postgres", () => {
  it("isolates tenants and denies memory after revoke or expiry under concurrent writes", async () => {
    ...
    const claims = JSON.stringify({ org_ids: [ORG_A] });
    ...
    const input = { consent_id: "consent-concurrent", organization_id: ORG_A, subject_ref: "contact-1", purpose: "support", channel: "email" as const, source_refs: ["form-1"], evidence_refs: ["event-1"] };
    try {
      ...
      await clientB.query("select set_config('request.jwt.claims',$1,false)", [JSON.stringify({ org_ids: [ORG_B] })]);
      expect(await new PostgresConsentRegistry(clientB).get(ORG_A, "consent-concurrent")).toBeUndefined();
      await expect(new PostgresConsentRegistry(clientB).revoke(ORG_A, "consent-concurrent")).rejects.toThrow("consent_not_found");
      const crossTenantUpdate = await clientB.query("update public.contact_consents set status='REVOKED' where organization_id=$1 and consent_id=$2", [ORG_A, "consent-concurrent"]);
      expect(crossTenantUpdate.rowCount).toBe(0);
      const scope = `company:${ORG_A}`;
      const write: ContactMemoryWrite = { owner: "agent-1", scope, authority: 3, organization_id: ORG_A, subject_ref: "contact-1", channel: "email", purpose: "support" };
      const policy: MemoryGatePolicy = { owner: "agent-1", scope, authority: 2 };
      ...
      await durable.revoke(ORG_A, "consent-concurrent");

...
async function waitForPostgres(url: string): Promise<void> {
  for (let i = 0; i < 40; i += 1) {
    try { const p = new Pool({ connectionString: url }); await p.query("select 1"); await p.end(); return; }
...
describe("Postgres Consent Registry (real RLS)", () => {
  ...
  it("persists, survives a new pool, revokes, and blocks cross-tenant access", async () => {
    const url = adminUrl.replace("postgres:test@", "consent_test:consent-test@");
    const pool = new Pool({ connectionString: url });
    const client = await pool.connect();
    try {
      await client.query(`SET app.org_ids = '${ORG_A}'`);
      ...
      expect(await registry.canContact(ORG_A, "contact-1", "email", "support")).toBe(true);
      expect(await registry.canContact(ORG_A, "contact-1", "voice", "support")).toBe(false);
    } finally { client.release(); await pool.end(); }

    const restarted = new Pool({ connectionString: url });
    const restartedClient = await restarted.connect();
    try {
      await restartedClient.query(`SET app.org_ids = '${ORG_A}'`);
      ...
      expect(await registry.canContact(ORG_A, "contact-1", "email", "support")).toBe(false);
    ...
    try {
      await crossClient.query(`SET app.org_ids = '${ORG_B}'`);
      ...
      const crossUpdate = await crossClient.query("UPDATE public.contact_consents SET status='GRANTED' WHERE organization_id=$1 AND consent_id='consent-1'", [ORG_A]);

...
export async function GET(request: Request) {
  try {
    ...
    if (!hasRequired) {
      return NextResponse.redirect(new URL('/app/content-os/accounts?error=permissions_missing', request.url))
    }
    ...
    await connectionRepo.upsertConnection({
      ...
      status: 'active',
      tokenExpiresAt: tokens.expiresAt ? new Date(tokens.expiresAt * 1000) : null,
      scopes: scopes,

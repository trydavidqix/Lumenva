...
const schema = z.object({ asset_type: z.string().trim().min(1).max(80), mime_type: z.string().trim().min(1).max(100), data_base64: z.string().min(1).max(140_000_000), content_item_id: z.string().uuid().nullable().optional(), origin_provider: z.string().trim().max(80).nullable().optional(), license: z.string().trim().max(120).nullable().optional(), provenance: assetProvenanceSchema.nullable().optional(), metadata: z.record(z.string(), z.unknown()).optional() }).strict();
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const asset = await registerContentAsset({ async create(input) {
      ...
      const { data, error } = await db.from("content_assets").insert(input as never).select("*").single(); if (error) throw error; return data as never; } }, { organizationId: authz.org.orgId, assetType: parsed.data.asset_type, mimeType: parsed.data.mime_type, bytes, contentItemId: parsed.data.content_item_id ?? null, originProvider: parsed.data.origin_provider ?? null, license: parsed.data.license ?? null, provenance: parsed.data.provenance ?? null, metadata: parsed.data.metadata ?? {} });
    void audit({ action: "content_os.asset_registered", actorUserId: authz.user.id, organizationId: authz.org.orgId, resourceType: "content_asset", resourceId: asset.id, requestId, metadata: { asset_type: asset.asset_type, mime_type: asset.mime_type, has_license: Boolean(asset.license), has_provenance: Boolean(asset.provenance) } });

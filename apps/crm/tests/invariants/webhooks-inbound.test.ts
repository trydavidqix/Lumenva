  ...
  sql(`
    ...
    do $guc$ begin
      execute format('alter database %I set app.nuvemshop_oauth_key = %L',
                     current_database(), 'test-guc-key-REDACTED_SECRETREDACTED_SECRET');
    end $guc$;
    select set_config('app.nuvemshop_oauth_key', 'test-guc-key-REDACTED_SECRETREDACTED_SECRET', false);
    insert into public.webhook_sources

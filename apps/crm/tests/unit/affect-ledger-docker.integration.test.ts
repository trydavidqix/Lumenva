...
describe("Wave 16 affect ledger with real PostgreSQL and RLS", () => {
  ...
  it("proves restart persistence, idempotency, append-only and tenant isolation", async () => {
    ...
    await expect(
      psql("select set_config('app.organization_id','org-b',true); update psyche_affect_events set pleasure=0 where event_id='event-1'", "app_user"),
    ).resolves.toMatch(/UPDATE 0$/);
    await expect(
      psql("select set_config('app.organization_id','org-a',true); update psyche_affect_events set pleasure=0 where event_id='event-1'", "app_user"),

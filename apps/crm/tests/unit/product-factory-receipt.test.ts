...
import { createDeliveryReceipt, getDeliveryReceipt, type DeliveryReceipt } from "@/lib/product-factory/receipt";
...
describe("delivery receipt", () => {
  it("creates an immutable audit receipt only after the delivery gate succeeds", async () => {
    ...
    expect(getDeliveryReceipt("receipt-1")).toBe(receipt);
  });
  ...
  it("refuses to issue a receipt when APPROVED/PACKAGED gate is absent", async () => {
    ...
    expect(getDeliveryReceipt("receipt-draft")).toBeUndefined();
  });
});

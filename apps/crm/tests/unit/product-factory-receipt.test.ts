...
describe("delivery receipt", () => {
  it("creates an immutable audit receipt only after the delivery gate succeeds", async () => {
    const receipt = await createDeliveryReceipt(plan, artifact, evidence, store(), receipts, { delivery_receipt_id: "receipt-1", actor_id: "actor-1", channel: "MANAGED_SERVICE", created_at: "2026-09-13T00:00:00.000Z" });
    ...
    expect(await getDeliveryReceipt(receipts, "org-1", "receipt-1")).toEqual(receipt);
  });

  it("refuses to issue a receipt when APPROVED/PACKAGED gate is absent", async () => {
    await expect(createDeliveryReceipt({ ...plan, status: "DRAFT" }, artifact, evidence, store(), receipts, { delivery_receipt_id: "receipt-draft", actor_id: "actor-1", channel: "MANAGED_SERVICE", created_at: "2026-09-13T00:00:00.000Z" })).rejects.toThrow("delivery plan must be APPROVED or PACKAGED");
    expect(await getDeliveryReceipt(receipts, "org-1", "receipt-draft")).toBeUndefined();
  });
});

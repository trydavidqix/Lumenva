...
const plan: DeliveryPlan = {
  delivery_plan_id: "receipt-plan-sec",
...
const artifact: DeliveryArtifact = {
  delivery_artifact_id: "artifact-sec",
...
describe("delivery receipt security", () => {
  it("rejects receipt ID reuse across tenants and freezes nested arrays", async () => {
    ...
    const receipt = await createDeliveryReceipt(plan, artifact, evidence, store(), receipts, {
      delivery_receipt_id: "receipt-sec-1",
      actor_id: "actor-1",
      channel: "MANAGED_SERVICE",
    ...
    await expect(createDeliveryReceipt(
      otherTenant,
      { ...artifact, delivery_plan_id: otherTenant.delivery_plan_id },
      otherEvidence,
      store(),
      receipts,
      {
    ...
    )).rejects.toThrow("receipt ID belongs to another tenant");
  ...
  it("rejects receipt reuse when the plan was reverted from PACKAGED to DRAFT", async () => {
    ...
    const receipt = await createDeliveryReceipt(
      { ...plan, delivery_plan_id: "receipt-plan-status" },
      { ...artifact, delivery_plan_id: "receipt-plan-status" },
      evidence,
      store(),
      receipts,
      {
        delivery_receipt_id: "receipt-status",
        actor_id: "actor-1",
        channel: "MANAGED_SERVICE",
        created_at: "2026-09-13T00:00:00.000Z",
      },
    ...
    await expect(createDeliveryReceipt(
      { ...plan, delivery_plan_id: "receipt-plan-status", status: "DRAFT" },
      { ...artifact, delivery_plan_id: "receipt-plan-status" },
      evidence,
      store(),
      receipts,
  ...
  it("rejects a receipt for a channel that is incompatible with the artifact platform", async () => {
    const receipts = receiptStore();
    await expect(createDeliveryReceipt(
      { ...plan, channels: ["APP_STORE"] },
      artifact,
      evidence,
      store(),
      receipts,
      {
        delivery_receipt_id: "receipt-platform",
        actor_id: "actor-1",
        channel: "APP_STORE",
        created_at: "2026-09-13T00:00:00.000Z",
      },
    )).rejects.toThrow("delivery channel APP_STORE requires IOS artifact");
    expect(await getDeliveryReceipt(receipts, "org-1", "receipt-platform")).toBeUndefined();
  });

  it("rejects same-tenant receipt ID replay when audit-significant inputs change", async () => {
    const receipts = receiptStore();
    const options = {
      delivery_receipt_id: "receipt-integrity",
      actor_id: "actor-1",
      channel: "MANAGED_SERVICE" as const,
      created_at: "2026-09-13T00:00:00.000Z",
    };
    await createDeliveryReceipt(plan, artifact, evidence, store(), receipts, options);

    await expect(createDeliveryReceipt(
      plan,
      artifact,
      { ...evidence, evidence_refs: ["different-evidence"] },
      store(),
      receipts,
      options,
    )).rejects.toThrow("receipt conflict does not match request");
    ...
    await expect(createDeliveryReceipt(
      ...
      store(),
      receipts,
      { ...options, created_at: "2026-09-13T01:00:00.000Z" },
    )).rejects.toThrow("receipt conflict does not match request");
  });
});

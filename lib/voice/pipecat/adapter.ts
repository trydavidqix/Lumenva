...
/**
 * Fase 1 scaffold only: shape of a Pipecat-backed runtime, mirroring
 * `PatterRuntime` so the factory can select either without the Agent OS
 * bridge (lib/voice/runtime/**) knowing which one is live. Real
 * faster-whisper/Piper/Kokoro/OpenVoice wiring (StreamingSttPort /
 * StreamingTtsPort implementations) lands in Fase 3 — this adapter does not
 * talk to a live Pipecat process yet.
 */
export type PipecatRuntimeEvent =
...
export interface PipecatRuntimeSession {
  events(): AsyncIterable<PipecatRuntimeEvent>;
...
/**
 * Internal seam around the Pipecat process. Nothing outside
 * `lib/voice/pipecat/**` needs to know how Pipecat represents calls or media.
 */
export interface PipecatRuntime {
  start(input: VoiceEngineStartInput): Promise<PipecatRuntimeSession>;
}

function mapRuntimeEvent(event: PipecatRuntimeEvent): VoiceEngineEvent {
  switch (event.kind) {
    case "speech_started":
...
function wrapSession(runtimeSession: PipecatRuntimeSession): VoiceEngineSession {
  return {
    async *events() {
...
export function createPipecatVoiceEngine(runtime: PipecatRuntime): VoiceEngine {
  return {

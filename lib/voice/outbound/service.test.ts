...
describe("governed voice outbound service", () => {
  it("accepts contact + goal, resolves phone server-side, gets Agent OS opening, then dials", async () => {
    const resolveContactPhone = vi.fn().mockResolvedValue("+351912345678");
    const resolveWorker = vi.fn().mockResolvedValue({ endpoint: "https://voice.internal" });
  ...
  it("does not dial when Agent OS governance blocks the opening", async () => {
    ...
    const service = createGovernedVoiceOutboundService({
      ...
      resolveWorker: vi.fn().mockResolvedValue({ endpoint: "https://voice.internal" }),

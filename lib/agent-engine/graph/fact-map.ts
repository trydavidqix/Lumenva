...
const CONSENT_PATTERN =
  /\b(consent(imento)?|opt[- ]?(in|out)|autoriza(?:ç|c)[aã]o (?:de|para) (?:contato|dados|marketing)|aceit(?:ou|e|ei)\s+(?:o|os)?\s*termos?)\b/iu;
const CONTRACT_PATTERN =
  /\b(contrato|contract|cl[aá]usula|clause|termos? de (?:uso|servi[cç]o)|terms? of service|acordo (?:legal|comercial)|\bnda\b|assinatura (?:do|de) contrato)\b/iu;
const PAYMENT_PATTERN =
  /\b(pagamento|payment|fatura|invoice|cobran[cç]a|billing|boleto|\bpix\b|cart[aã]o de cr[eé]dito|credit card|inadimplent\w*|past[- ]due|chargeback|reembolso|refund)\b/iu;
 ...
 * `"high"` here, independent of anything Graphiti reports. The remaining
 * domains get a lower, still-conservative default; `relationship` and
 * `customer_preference` are explicitly allowed a lower tier per the same
 * constraint ("customer preference/relationship facts may map lower risk").
 * This is the single source of truth for risk-by-domain in this mapper —
 * `mapGraphFact` never reads `fact.risk` from the input, see its doc comment
 * for why.
 */

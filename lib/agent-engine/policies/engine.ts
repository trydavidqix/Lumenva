import type { AgentAutonomyLevel as ContractAutonomyLevel } from "../contracts/agent-os";
import type { AgentToolDefinition } from "../tools/registry";

export type AgentAutonomyLevel = ContractAutonomyLevel;
...
export function evaluateToolPolicy(input: EvaluateToolPolicyInput): PolicyDecision {
  ...
  if (includesTool(tenantPolicy.approvalToolIds, input.tool.id) || includesTool(agentPolicy.approvalToolIds, input.tool.id)) {
  ...
  switch (input.autonomyLevel) {
    ...
    case "assisted":
      ...
      if (input.tool.risk === "r1_reversible_write") {
        return input.promotionEvidencePassed === true
          ? { kind: "allow" }
    ...
    case "autopilot_low_risk":
      if (input.tool.risk === "r0_read" || input.tool.risk === "r1_reversible_write") return { kind: "allow" };
      return approval("autonomy_level_requires_approval", "external_communication");

...
export async function runAgentTurn(
  ...
  if (deps.tracer !== undefined) {
    try {
      ...
      // Tracing must never prevent an agent turn.
    }
  }

  let traceEnd: Parameters<AiTraceSpan["end"]>[0] = {};
  try {
    ...
    if (!openingContext.ok) {
      ...
      throw new Error(
        `abertura do turno falhou em get_lead_context (${openingContext.error.code})`,
      );
    }
    ...
    if (
      ...
      runLog.info(
        "handoff humano acionado por pedido explícito do lead (detecção determinística)",
        {
          kind: job.kind,
        },
      );
      ...
      get_lead_note: tool({
        ...
        execute: async ({ note_id }) => {
          try {
            ...
            const body =
              noteId === "" ? null : await getLeadNoteBody(pool, tenantId, leadId, noteId);
            if (body === null) {
              return {
                ok: false,
                error: {
    ...
    // Errors can contain model, CRM, or lead data. The trace gets a stable code only.
    traceEnd = { error: "agent_turn_failed" };
    throw error;
  } finally {
    await endTraceSpan(span, traceEnd);
  }
}
...
async function endTraceSpan(
  ...
  try {
    ...
    // Tracing must never change agent-turn behavior.
  }
}

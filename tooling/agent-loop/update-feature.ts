// update-feature.ts — a ÚNICA porta de escrita do loop em docs/product/plans/features.json.
//
// Uso: node tooling/agent-loop/update-feature.ts --id G1-01 --passes true --verification '<json>'
//
...
// e o pre-commit (tooling/agent-loop/hooks/validate-features.sh) revalida o diff de qualquer forma.
//
...
//   Escolhido assim para manter o comando canônico `node tooling/agent-loop/update-feature.ts`
...
const FEATURES_PATH = join(__dirname, "..", "..", "docs", "product", "plans", "features.json");
...
function fail(msg: string): never {
  ...
  console.error("Uso: node tooling/agent-loop/update-feature.ts --id <ID> [--passes true|false] [--verification '<json>']");
  process.exit(1);
}
...
if (!Array.isArray(plan.features)) fail("docs/product/plans/features.json sem array .features.");

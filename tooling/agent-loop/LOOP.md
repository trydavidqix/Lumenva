...
> fases em `docs/product/plans/phases.md`). Você não tem memória de sessões anteriores e não terá
...
`tooling/agent-loop/inbox.items.md`.

---

## 0. Guardas de entrada (antes de qualquer coisa — qualquer uma falhou, SAIA)

1. `tooling/agent-loop/STOP` existe → saia imediatamente. (Kill switch do dono.)
2. Existe `tooling/agent-loop/checkpoints/G<N>-report.md` SEM o par `tooling/agent-loop/checkpoints/G<N>.approved`
   → há checkpoint aguardando aprovação humana. Saia. O loop não anda com gate aberto.
   Existe `tooling/agent-loop/checkpoints/G<N>.rejected` → a fase foi RECUSADA pelo dono e está
   ...
   features apontadas pelo dono (via `node tooling/agent-loop/update-feature.ts`) e remover o
   ...
   sobre a recusa → saia.
3. **Teto diário**: conte as linhas `started` de HOJE em `tooling/agent-loop/sessions.log`.
   "Hoje" é no fuso **America/Sao_Paulo** — explícito, porque a virada de dia em UTC
   às 21h locais mudaria o orçamento:
   ```bash
   grep -c "^$(TZ=America/Sao_Paulo date +%F).* started$" tooling/agent-loop/sessions.log
   ```
   Se ≥ `max_sessions_per_day` de `tooling/agent-loop/loop.config.json` (default 12) → saia.
   (Teto de custo/dia é física, não sugestão.)
4. Lock de lane: a lane única é **core**. `tooling/agent-loop/locks/core.lock` com timestamp < 2h
...
5. **Registre a abertura**: acrescente a `tooling/agent-loop/sessions.log` uma linha
...
6. **Chão limpo**: `git status --porcelain` não-vazio → uma sessão anterior morreu
   ...
   `docs/product/plans/progress.md` ("tree sujo na entrada, stashado como orphan <ISO>"). O
...
> (`tooling/agent-loop/loop.config.json → main_checkout`, hoje `/Users/rafaelmelgaco/DeskcommCRM`).
...
2. Leia as últimas ~40 linhas de `docs/product/plans/progress.md`.
3. Leia `docs/product/plans/features.json` inteiro.
4. Leia `tooling/agent-loop/inbox.items.md` — features com item **aberto** estão CONGELADAS pra
   você (regra anti-apodrecimento em `tooling/agent-loop/INBOX.md`). **Exceção única**: item tipo
   `transient` com `sessions_seen ≤ 2` — UMA re-tentativa é permitida (INBOX.md
   regras 2-3; incremente `sessions_seen` ao tentar).
5. **Virada de fase pendente?** Se existe `tooling/agent-loop/checkpoints/G<N>.approved` E
...
6. **Determine a fase corrente**: a menor fase G* (G1→G6) com alguma feature
   ...
   G<n> (n≥2) só é elegível se `tooling/agent-loop/checkpoints/G<n-1>.approved` existe. (O gate
...
8. **Smoke ANTES de implementar**: `pnpm typecheck && pnpm lint && pnpm test:unit`
   (o comando canônico vive em `tooling/agent-loop/loop.config.json → smoke`). **Vermelho? A main
...
- **Briefing ao subagente** (padrão dispatch — contexto completo, uma tarefa):
  ...
  - **Migrations em TRIPLA**: `infra/supabase/migrations/<ts>_<NNNN>_<slug>.sql`
    idempotente + apêndice em `infra/supabase/baseline.sql` + linha em
    `infra/supabase/migrations/MANIFEST.md` + `lib/database.types.ts` regenerado.
    O NNNN é verificado contra TODAS as branches locais (a cadeia `vendaval/F2-*`
    tem migrations não mergeadas) — o hook `tooling/agent-loop/hooks/check-migration-triple.sh`
...
- **Feature com superfície de UI**: o briefing exige screenshot da tela entregue em
  `tooling/agent-loop/checkpoints/evidence/<fase>/` (versionado — é a evidência do checkpoint).
...
2. **`docs/product/plans/features.json`**: marque a feature via
   ```bash
   node tooling/agent-loop/update-feature.ts --id <FEATURE-ID> --passes true \
   ...
   **NUNCA com Edit/Write direto** — o hook PreToolUse bloqueia, e o pre-commit
   (`tooling/agent-loop/hooks/validate-features.sh`) revalida o diff: só `passes`/`verification`
...
3. **`docs/product/plans/progress.md`**: acrescente 3-5 linhas — data/sessão, feature, decisão
   não-óbvia tomada (se houve), o que a próxima sessão precisa saber. É um diário
   de bordo, não um relatório.
4. **`tooling/agent-loop/sessions.log`**: a linha de RESULTADO, par da linha `started` do §0.5:
   `<ISO America/Sao_Paulo> core <FEATURE-ID|repair|blocked|idle> <resultado>`.
5. Remova seu `tooling/agent-loop/locks/core.lock`.
...
1. Escreva `tooling/agent-loop/checkpoints/<FASE>-report.md` seguindo `tooling/agent-loop/CHECKPOINT.md`
...
4. **PARE.** Não escolha outra feature, não espie a próxima fase. A guarda de
   ...
   `tooling/agent-loop/checkpoints/<FASE>.approved` (ou bloqueadas pelo `.rejected`, se recusar).
   O que acontece na aprovação/recusa está em `tooling/agent-loop/CHECKPOINT.md`.
...
- **Nunca `git push`.** Push só existe no ritual de virada de fase (CHECKPOINT.md).
  E não é só instrução: o hook `tooling/agent-loop/hooks/pre-push` recusa qualquer push sem
...
- **Migration fora da tripla não commita.** `tooling/agent-loop/hooks/check-migration-triple.sh`
...
---
*Contrato de `docs/product/plans/features.json` (escrito pela lane de features; o loop só consome —

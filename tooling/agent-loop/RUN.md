...
O loop é o mesmo em qualquer modo: uma sessão descartável executa `tooling/agent-loop/LOOP.md`
...
(`tooling/agent-loop/loop.config.json → main_checkout`) — DEPOIS que a branch `gov/setup` for
...
- Repo com `main` já contendo `gov/setup`; `nvm use && pnpm install`; `jq`
  ...
  (`node tooling/agent-loop/update-feature.ts` usa type-stripping nativo); docker (a suíte
  `tests/invariants` usa Postgres descartável).
- **Hooks determinísticos** (a parte do protocolo que não é prosa):
  ```bash
  bash tooling/agent-loop/setup-hooks.sh
  ```
...
- **Guard PreToolUse do Claude Code** (merge ADITIVO no `.claude/settings.local.json`
  ...
  ```
  Arma o bloqueio de Edit/Write em `docs/product/plans/features.json` (mutação só via
  `node tooling/agent-loop/update-feature.ts`), de Edit/Write em invariante existente e de
...
- **`.gitignore` do loop** (estado operacional fica fora do git — senão a resposta
  ...
  (`tooling/agent-loop/inbox.items.md`, `tooling/agent-loop/sessions.log`, `tooling/agent-loop/logs/`, `tooling/agent-loop/locks/`).
  **`tooling/agent-loop/checkpoints/evidence/` NÃO é ignorado** — screenshot de checkpoint é
  evidência versionada.
- `docs/product/plans/features.json` populado (26 features G1-01..G6-04); `docs/product/plans/progress.md`
  existe; `tooling/agent-loop/loop.config.json`:
  ```json
  {
...
Salve como `tooling/agent-loop/run-session.sh` (não versionado por default — crie quando for
...
[ -f tooling/agent-loop/STOP ] && exit 0
# gate barato em bash: checkpoint pendente ou recusado => nem invoca o modelo
for r in tooling/agent-loop/checkpoints/*-report.md; do
  [ -e "$r" ] || continue
  [ -f "${r%-report.md}.approved" ] || exit 0
done
for rej in tooling/agent-loop/checkpoints/*.rejected; do
...
mkdir -p tooling/agent-loop/logs
claude -p "Leia tooling/agent-loop/LOOP.md e execute o protocolo à risca." \
  --permission-mode acceptEdits \
  --allowedTools "Read,Edit,Write,Bash,Grep,Glob,Task" \
  >> "tooling/agent-loop/logs/$(date +%Y%m%d-%H%M%S)-core.log" 2>&1
```
...
`/bin/bash -lc '/Users/rafaelmelgaco/DeskcommCRM/tooling/agent-loop/run-session.sh'` e
...
1. **Nunca `git push` sem checkpoint aprovado — enforcement físico**: o hook
   `tooling/agent-loop/hooks/pre-push` recusa qualquer push sem `DESKCOMM_GOV_PHASE_MERGE=1`, e
...
3. **Teto de sessões/dia** (`max_sessions_per_day: 12`) checado pela própria sessão
   via linhas `started` de hoje (America/Sao_Paulo) em `tooling/agent-loop/sessions.log` — a
   linha é escrita na ABERTURA, então sessão que morre no meio conta no orçamento.
4. **Kill switch**: `touch tooling/agent-loop/STOP` para o loop na próxima sessão (e o script nem
   invoca o modelo). `rm tooling/agent-loop/STOP` religa.
5. **Imutabilidade do plano — enforcement físico**: o pre-commit
   (`tooling/agent-loop/hooks/validate-features.sh`) rejeita commit que altere qualquer campo de
   `docs/product/plans/features.json` além de `passes`/`verification` — a menos que
   `DESKCOMM_GOV_PLAN_EDIT=1` (sessão humana). E a mutação legítima é SÓ via
   `node tooling/agent-loop/update-feature.ts` (o PreToolUse barra Edit/Write direto).
...
- `tail -f tooling/agent-loop/logs/<mais recente>` — a sessão corrente.
- `git log --oneline gov/G1` — o que já entrou.
- `docs/product/plans/progress.md` — o diário. `tooling/agent-loop/inbox.items.md` — o que precisa de você.
- `tooling/agent-loop/sessions.log` — aberturas e resultados (started sem par = sessão que morreu).
- `jq '[.features[] | select(.passes)] | length' docs/product/plans/features.json` — placar (de 26).

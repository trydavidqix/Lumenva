...
deny() {
  echo "BLOQUEADO: $1" >&2
  echo "Se isso bloqueia sua feature, abra um item em tooling/agent-loop/inbox.items.md (formato em tooling/agent-loop/INBOX.md) e devolva BLOCKED." >&2
  exit 2
}
...
case "$tool" in
  Edit|Write)
    ...
    case "$path" in
      *docs/product/plans/features.json)
        deny "docs/product/plans/features.json não se edita com Edit/Write — mutação de passes/verification só via 'node tooling/agent-loop/update-feature.ts'."
        ;;
      *tests/invariants/*)

...
# Commit que ADICIONA arquivo em infra/supabase/migrations/*.sql precisa, no MESMO commit:
#   1. mudança em infra/supabase/baseline.sql (apêndice idempotente)
#   2. mudança em infra/supabase/migrations/MANIFEST.md (linha na tabela Applied)
...
new_migrations=$(git diff --cached --name-status \
  | awk '$1 == "A" && $2 ~ /^infra\/supabase\/migrations\/.*\.sql$/ { print $2 }')
[ -z "$new_migrations" ] && exit 0

staged=$(git diff --cached --name-only)

if ! grep -qx 'infra/supabase/baseline.sql' <<<"$staged"; then
  echo "pre-commit BLOQUEADO: migration nova sem apêndice em infra/supabase/baseline.sql no MESMO commit." >&2
...
if ! grep -qx 'infra/supabase/migrations/MANIFEST.md' <<<"$staged"; then
  echo "pre-commit BLOQUEADO: migration nova sem linha em infra/supabase/migrations/MANIFEST.md no MESMO commit." >&2
  echo "A tripla é indivisível (CLAUDE.md §Migrations): migrations/*.sql + baseline.sql + MANIFEST.md." >&2
...
while IFS= read -r path; do
  ...
  while IFS= read -r branch; do
    conflict=$(git ls-tree -r --name-only "$branch" -- infra/supabase/migrations supabase/migrations 2>/dev/null \
      | grep -E "^(infra/)?supabase/migrations/[0-9]+_${nnnn}_.+\.sql$" || true)
    if [ -n "$conflict" ]; then

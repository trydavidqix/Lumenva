...
Arquivo: `tooling/agent-loop/checkpoints/<FASE>-report.md` (ex.: `G1-report.md`), commitado na
...
Feature com superfície de UI: screenshots em tooling/agent-loop/checkpoints/evidence/<FASE>/
referenciados aqui.
...
### Gates específicos por fase (de `docs/product/plans/phases.md` — o relatório DEVE evidenciá-los)
...
1. **Aprovar**: criar o arquivo de aprovação e commitá-lo —
   ```bash
   echo "approved $(date -Iseconds) by Rafael — <observação opcional>" > tooling/agent-loop/checkpoints/<FASE>.approved
   git add tooling/agent-loop/checkpoints/<FASE>.approved && git commit -m "chore(<FASE>): checkpoint aprovado"
   ```
...
3. **Recusar**: NÃO criar o `.approved`. Em vez disso:
   ```bash
   echo "rejected $(date -Iseconds) by Rafael — <motivo em 1 linha>" > tooling/agent-loop/checkpoints/<FASE>.rejected
   git mv tooling/agent-loop/checkpoints/<FASE>-report.md tooling/agent-loop/checkpoints/<FASE>-report.rejected-$(date +%F).md
   git add tooling/agent-loop/checkpoints/<FASE>.rejected && git commit -m "chore(<FASE>): checkpoint recusado"
   ```
   ...
   reabrir `passes:false` nas features apontadas (via `node tooling/agent-loop/update-feature.ts`)
...
está pendente quando `tooling/agent-loop/checkpoints/<FASE>.approved` existe E
...
gh pr create --base main --head gov/<FASE> \
  ...
  --body "Report: tooling/agent-loop/checkpoints/<FASE>-report.md — aprovado em tooling/agent-loop/checkpoints/<FASE>.approved"

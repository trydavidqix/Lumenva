...
describe("Postgres Consent Registry (real RLS)", () => {
  ...
  it("persiste consentimento, reconstrói após nova pool, revoga e impede cross-tenant", async () => {
    ...
    try {
      ...
      const crossUpdate = await crossClient.query("UPDATE public.contact_consents SET status='GRANTED' WHERE organization_id=$1::uuid AND consent_id='consent-1'", [ORG_A]);
